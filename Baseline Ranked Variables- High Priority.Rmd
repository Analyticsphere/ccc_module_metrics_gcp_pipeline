...
## HORNONCISWOM
```{r HORNONCISWOM, warning=FALSE, echo=FALSE, message=FALSE}
...
agep <- hormn %>%  mutate(horm= case_when(D_798889867=="200323667"~"In the past yes",
                                      D_798889867=="249673703"~"Currently yes",
                                      D_798889867=="104430631"~"No",
                                      TRUE ~ "Skipped this Question"))

dt_agep_summary <- agep %>% group_by(horm) %>%  dplyr::summarize(n=n(), percentage=100*n/nrow(.))  %>% dplyr::ungroup() %>%  dplyr::select(horm, n, percentage)

#gt_total_func(dt_agep_summary, "horm", "HORNONCISWOM: Used Hormones to Make Body Look More Feminine")
dt_agep_summary %>%  gt::gt(rowname_col = "row_lab") %>%  
  fmt_number(columns = "percentage", decimals = 2) %>% 
  fmt_number(column = "n", decimals=0, use_seps = TRUE) %>% 
  tab_header(title = md("HORNONCISWOM: Used Hormones to Make Body Look More Feminine")) %>% 
  cols_label(n= md("**N**"), horm = md("**Answer**"), percentage = md("**%**")) %>% 
  grand_summary_rows(columns=c(n, percentage),fns = ~sum(.,na.rm = T))|>
  tab_options(
      stub.font.weight = "bold"
    ) %>%

...
```{r SJ_Adhocs, echo=FALSE, message=FALSE, warning=FALSE, include=FALSE}


######################## Ad-hoc request for Sex, Sexual Orientation, and Style

style_ctabs= data_tib_m1  %>%  mutate(gender= case_when(D_289664241_D_289664241==536341288 | D_289664241_D_289664241==218837028 ~ "Woman",
                                               D_289664241_D_289664241==654207589 | D_289664241_D_289664241==983318667 ~ "Man",
                                               D_289664241_D_289664241==405267600 ~ "Transgender Man",
                                               D_289664241_D_289664241==873138103 ~ "Transgender Woman",
                                               D_289664241_D_289664241==805712793 ~ "Genderqueer",
                                ...
                                sex=case_when(D_407056417=="536341288" ~ "Female",
                                              ...
                                              D_407056417=="576796184" ~ "Unknown", ##deprecated previous response
                                              TRUE ~ "Skipped this Question"),
                                sex_ornt = case_when(D_555481393_D_555481393=="271882746"~"Straight or heterosexual", 
                                                     ...
                                                     TRUE ~ "Skipped this Question"),
                                style=case_when(D_987107433=="220083334"~"Very Feminine",
                                                ...
                                                TRUE ~ "Skipped this Question"),
                                      gender=factor(gender, levels=c("Woman","Man","Transgender Man","Transgender Woman", "Genderqueer", "Non-binary", 
                                                                     "Other", "Prefer Not to Answer", "Skipped this Question")),
                                      sex_ornt=factor(sex_ornt, levels=c("Straight or heterosexual",  "Lesbian, gay, or homosexual",  "Bisexual",  
                                                                         "Queer",  "No labels",  "Asexual",  "Mostly Straight", "Pansexual", "Skipped this Question")),
                                      style=factor(style, levels=c("Very Feminine", "Mostly Feminine", "Somewhat Feminine", "Equally feminine and masculine", 
                                                                   "Very masculine", "Mostly Masculine","Somewhat Masculine", "Skipped this Question")))

create_tbl_cross1 <- function(data,row_var, colm_var, row_or_col_pct, row_name, column_name){
    cross__table <- data %>%  select(row_var, colm_var) %>% 
        tbl_cross(
            row = !!sym(row_var),
            col = !!sym(colm_var),
            digits=c(0,2),
...
style_by_sex = create_tbl_cross1(style_ctabs, "style", "sex", "row", "Style", "Sex at Birth")
colnames(style_by_sex)[1] <- "Style"
total_rows <- nrow(style_by_sex)
knitr::kable(style_by_sex , 
             caption='Distribution of Style by Sex at Birth', 
...
style_by_gender = create_tbl_cross1(style_ctabs, "style", "gender", "row", "Style", "Gender")
colnames(style_by_gender)[1] <- "Style"
total_rows <- nrow(style_by_gender)
knitr::kable(style_by_gender , 
             caption='Distribution of Style by Gender', 
             row.names=FALSE,align=c("l","c","c","c","c"),
             booktabs = TRUE) %>%  
  add_indent(seq(1, total_rows - 1))  %>% 
  kable_styling(latex_options = c("scale_down","hold_position")) # %>%  column_spec(3:4, width = "4cm")
...
style_by_sex_ornt = create_tbl_cross1(style_ctabs, "style", "sex_ornt", "row", "Style", "Sexual Orientation")
colnames(style_by_sex_ornt)[1] <- "Style"
total_rows <- nrow(style_by_sex_ornt)
knitr::kable(style_by_sex_ornt , 
             caption='Distribution of Style by Sexual Orientation', 
             row.names=FALSE,align=c("l","c","c","c","c"),
             booktabs = TRUE) %>%  
  add_indent(seq(1, total_rows - 1))  %>% 
  kable_styling(latex_options = c("scale_down","hold_position")) # %>%  column_spec(3:4, width = "4cm")
...
style_lsb <- style_ctabs %>% filter(sex== "Female" & sex_ornt=="Lesbian, gay, or homosexual") %>%  
  mutate(sex_ornt=droplevels(sex_ornt))
style_of_lesbian_female = create_tbl_cross1(style_lsb, "style", "sex_ornt", "column", "Style", "Sexual Orientation")
colnames(style_of_lesbian_female)[1] <- "Style"
total_rows <- nrow(style_of_lesbian_female)
knitr::kable(style_of_lesbian_female, 
             caption='Distribution of Style Among Female Lesbians', 
             row.names=FALSE,align=c("l","c","c","c","c"),
             booktabs = TRUE) %>%  
  add_indent(seq(1, total_rows - 1))  %>% 
...
tmpM2 <- tmpM2 %>%  select( -"D_173240848_D_195093589") #-"D_173240848_D_173240848_D_195093589", -"D_173240848_D_173240848_D_210004486")

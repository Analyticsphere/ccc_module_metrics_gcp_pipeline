...
child_logic_funct <- function(CID, Child_Num, Title){  
  ...
  gt_yn <- dt_select  %>% dplyr::group_by(!!CID_SYM) %>% dplyr::summarize(n=n(), percentage=100*n/nrow(.)) %>% dplyr::ungroup() %>% dplyr::mutate(answer=recode(!!CID_SYM, !!!dict)) %>%
    ...
    dplyr::select(answer, n, percentage)
...
age_summary <- data_tib_m1 %>% group_by(age) %>%  dplyr::summarize(n=n(), percentage=100*n/nrow(.))  %>% dplyr::ungroup() %>%  dplyr::select(age, n, percentage)

age_summary %>%  gt::gt() %>%  
  fmt_number(columns = "percentage", decimals = 2) %>% 
  tab_header(title = md("Age Ranges of Participants")) %>% 
...
dt_am_ind_ethn_summary %>%  gt::gt(rowname_col = "row_lab") %>%
  ...
  tab_footnote(
    footnote = md("The denomination of this table is all participants who selected this race/ethnicity only, as well as multi-racial participants that selected this race/ethnicity as well as other race/ethnicities."),
...
sex_table %>%
  tab_footnote(
    footnote = md("This became an optional question on 1/31/2025."),
  placement = c("left")
) 
...
dt_sex2= data_tib_m1 %>%  mutate(multb2 = ifelse(D_750420077_D_846483618==1, 1, 0) + ifelse(D_750420077_D_505282171==1, 1, 0) + ifelse(D_750420077_D_578416151==1, 1, 0) + 
...
dt_sex2_summary %>%  gt::gt(rowname_col = "row_lab") %>%  
  fmt_number(columns = "percentage", decimals = 2) %>% 
  tab_header(title = md("Body Part Participants Were Born With")) %>% 
  ...
  tab_footnote(
    footnote = md("This became a required question on 1/31/2025."),
  placement = c("left")
)  %>%
  tab_footnote(
    footnote = md("'Multiple Body Parts' is defined as a participant selecting more than one body part."),
    locations = cells_column_labels(columns = body_part),
  placement = c("left")
) 
...
connect_id_map <- c(
  "D_517307064_D_517307064_D_752953170" = "Breast Implants",
  "D_517307064_D_517307064_D_325506683" = "Mastectomy",
  "D_517307064_D_517307064_D_335563082" = "Breast Reconstruction",
  "D_517307064_D_517307064_D_955881350" = "Breast Reduction",
  "D_517307064_D_517307064_D_492902023" = "Partial Mastectomy",
  "D_517307064_D_517307064_D_256196714" = "Mastectomy",
  "D_517307064_D_517307064_D_802859122" = "Double Mastectomy",
  "D_517307064_D_517307064_D_520432394" = "Abscess",
  "D_517307064_D_517307064_D_667901971" = "Removal of Lactiferous",
  "D_517307064_D_517307064_D_807835037" = "Other",
  "D_517307064_D_517307064_D_535003378" = "None"
)


tmp_Brst <- data_tib_m1 %>% filter(D_624179836_D_533491176 == 1)


tmpMC2 <- tmp_Brst %>% select(starts_with("D_517307064_")) 

# Drop the text box option
tmpM2 <- tmpMC2 %>% select(-D_517307064_D_942347130)

# Ensure all columns in tmpM2 are numeric (convert TRUE/FALSE to 1/0 if needed)
tmpM2[] <- lapply(tmpM2, function(x) as.numeric(as.character(x)))

Bsurgnc_dx <- data.frame(
  Condition = connect_id_map[colnames(tmpM2)],  # Map column names to labels
  Frequency = colSums(tmpM2, na.rm = TRUE)
)


Bsurgnc_dx$Percentage <- round((Bsurgnc_dx$Frequency / nrow(tmp_Brst)) * 100, 2)


skipped_mhg1 <- sum(is.na(rowSums(tmpM2)))  # Count rows where all responses are NA
Bsurgnc_dx <- rbind(
  Bsurgnc_dx,
  data.frame(Condition = "Skipped this Question", Frequency = skipped_mhg1, 
             Percentage = round((skipped_mhg1 / nrow(data_tib_m1)) * 100, 2))
)


dt_surgeryBrst <- Bsurgnc_dx %>% 
  gt::gt() %>%
  tab_header(title = md("BREASTSUR: Types of Breast Surgeries Had")) %>%
  cols_label(Frequency = md("**N**"), Percentage = md("**Percentage of Participants**"), Condition = md("**Answer**")) %>%
...
connect_id_map <- c(
    "D_814664694_D_814664694_D_903825143" = "Rogaine, Minoxidil, or Ioniten",
    "D_814664694_D_814664694_D_567365088" = "Finasteride",
    "D_814664694_D_814664694_D_807835037" = "Other"
)
...
tmp_hrm2 <- dt_hrmed_typ %>% select(starts_with("D_814664694"))

# Drop the text box option
tmp_hrm2 <- tmp_hrm2 %>% select(-D_814664694_D_787193493)

# Ensure all columns in tmp_hrm2 are numeric (convert TRUE/FALSE to 1/0 if needed)
tmp_hrm2[] <- lapply(tmp_hrm2, function(x) as.numeric(as.character(x)))


hrm2_dx <- data.frame(
    Condition = connect_id_map[colnames(tmp_hrm2)],  # Map column names to labels
    Frequency = colSums(tmp_hrm2, na.rm = TRUE)
)


hrm2_dx$Percentage <- round((hrm2_dx$Frequency / nrow(dt_hrmed_typ)) * 100, 2)


skipped_mhg1 <- sum(is.na(rowSums(tmp_hrm2)))  
hrm2_dx <- rbind(
    hrm2_dx,
    data.frame(Condition = "Skipped this Question", Frequency = skipped_mhg1, 
               Percentage = round((skipped_mhg1 / nrow(data_tib_m1)) * 100, 2))
...
# Create a map for cancer types
cancer_map <- c(
    "D_173240848_D_173240848_D_939782495" = "Anal",
    "D_173240848_D_173240848_D_135725957" = "Bladder",
    "D_173240848_D_173240848_D_518416174" = "Brain",
    "D_173240848_D_173240848_D_847945207" = "Breast",
    "D_173240848_D_173240848_D_942970912" = "Colon/Rectal",
    "D_173240848_D_173240848_D_596122041" = "Esophageal",
    "D_173240848_D_173240848_D_489400183" = "Head, Neck, and Shoulders",
    "D_173240848_D_173240848_D_863246236" = "Kidney",
    "D_173240848_D_173240848_D_607793249" = "Leukemia",
    "D_173240848_D_173240848_D_532172400" = "Liver",
    "D_173240848_D_173240848_D_754745617" = "Lung",
    "D_173240848_D_173240848_D_665036297" = "Non-Hodgkin's Lymphoma",
    "D_173240848_D_173240848_D_200837530" = "Lymphoma",
    "D_173240848_D_173240848_D_990319383" = "Melanoma Skin",
    "D_173240848_D_173240848_D_487917585" = "Non-Melanoma Skin",
    "D_173240848_D_173240848_D_482225200" = "Pancreatic",
    "D_173240848_D_173240848_D_295976386" = "Prostate",
    "D_173240848_D_173240848_D_764891959" = "Stomach",
    "D_173240848_D_173240848_D_248374037" = "Testicular",
    "D_173240848_D_173240848_D_139822395" = "Thyroid",
    "D_173240848_D_173240848_D_807835037" = "Other",
    "D_173240848_D_173240848_D_178420302" = "Unknown"
)

# Filter the dataset
dt_dad_cnc <- data_tib_m1 %>% filter(D_659784914 == 353358909)

# Select only columns related to cancer diagnosis
tmp_dad_cnc <- dt_dad_cnc %>% select(starts_with("D_173240848"))

# Drop the text box option (the D_173240848_D_195093589 column)
tmp_dad_cnc <- tmp_dad_cnc %>% select(-D_173240848_D_195093589, -D_173240848_D_173240848_D_210004486)

# Ensure all columns are numeric (converting TRUE/FALSE to 1/0)
tmp_dad_cnc[] <- lapply(tmp_dad_cnc, function(x) as.numeric(as.character(x)))

# Summarize the data: Calculate frequency and map to conditions
dad_cnc_dx <- data.frame(
    Condition = cancer_map[colnames(tmp_dad_cnc)],  # Map column names to labels
    Frequency = colSums(tmp_dad_cnc, na.rm = TRUE)
)

# Calculate the percentage of participants
dad_cnc_dx$Percentage <- round((dad_cnc_dx$Frequency / nrow(dt_dad_cnc)) * 100, 2)

# Count skipped responses
skipped_mhg1 <- sum(is.na(rowSums(tmp_dad_cnc)))
dad_cnc_dx <- rbind(
    dad_cnc_dx,
    data.frame(Condition = "Skipped this Question", Frequency = skipped_mhg1, 
               Percentage = round((skipped_mhg1 / nrow(data_tib_m1)) * 100, 2))
...
sib_logic_funct(D_579563781_2_2, 2, "SIB2_2: Sibling 2 Sex")
sib_logic_funct(D_579563781_3_3, 3, "SIB2_3: Sibling 3 Sex")
sib_logic_funct(D_579563781_4_4, 4, "SIB2_4: Sibling 4 Sex")
sib_logic_funct(D_579563781_5_5, 5, "SIB2_5: Sibling 5 Sex")
sib_logic_funct(D_579563781_6_6, 6, "SIB2_6: Sibling 6 Sex")
sib_logic_funct(D_579563781_7_7, 7, "SIB2_7: Sibling 7 Sex")
sib_logic_funct(D_579563781_8_8, 8, "SIB2_8: Sibling 8 Sex")
...
child_logic_funct(D_887697329_2_2, 2, "CHILD2_2: Child 2 Sex")
child_logic_funct(D_887697329_3_3, 3, "CHILD2_3: Child 3 Sex")
child_logic_funct(D_887697329_4_4, 4, "CHILD2_4: Child 4 Sex")
child_logic_funct(D_887697329_5_5, 5, "CHILD2_5: Child 5 Sex")
child_logic_funct(D_887697329_6_6, 6, "CHILD2_6: Child 6 Sex")
child_logic_funct(D_887697329_7_7, 7, "CHILD2_7: Child 7 Sex")
child_logic_funct(D_887697329_8_8, 8, "CHILD2_8: Child 8 Sex")
...
child1= dt_ch1_d_age  %>%  mutate(oth_age= case_when(!is.na(D_640010727_1_1_D_299300933_1) ~ "Answered with Age",
...
child2= dt_ch2_d_age  %>%  mutate(oth_age= case_when(!is.na(D_640010727_2_2_D_299300933_2) ~ "Answered with Age",
...
child3= dt_ch3_d_age  %>%  mutate(oth_age= case_when(!is.na(D_640010727_3_3_D_299300933_3) ~ "Answered with Age",
...
child4= dt_ch4_d_age  %>%  mutate(oth_age= case_when(!is.na(D_640010727_4_4_D_299300933_4) ~ "Answered with Age",
...
child5= dt_ch5_d_age  %>%  mutate(oth_age= case_when(!is.na(D_640010727_5_5_D_299300933_5) ~ "Answered with Age",
...
child6= dt_ch6_d_age  %>%  mutate(oth_age= case_when(!is.na(D_640010727_6_6_D_299300933_6) ~ "Answered with Age",
...
child1= dt_chd1_d_age  %>%  mutate(oth_age= case_when(!is.na(D_236590500_1_1_D_182786508_1) ~ "Answered with age",
...
child2= dt_chd2_d_age  %>%  mutate(oth_age= case_when(!is.na(D_236590500_2_2_D_182786508_2) ~ "Answered with age",
...
child3= dt_chd3_d_age  %>%  mutate(oth_age= case_when(!is.na(D_236590500_3_3_D_182786508_3) ~ "Answered with age",
...
child4= dt_chd4_d_age  %>%  mutate(oth_age= case_when(!is.na(D_236590500_4_4_D_182786508_4) ~ "Answered with age",
...
child5= dt_chd5_d_age  %>%  mutate(oth_age= case_when(!is.na(D_236590500_5_5_D_182786508_5) ~ "Answered with age",
...
child6= dt_chd6_d_age  %>%  mutate(oth_age= case_when(!is.na(D_236590500_6_6_D_182786508_6) ~ "Answered with age",
...
kid_bd_canc <- dt_body_kid_summary %>%  gt::gt(rowname_col = "row_lab") %>%  
  fmt_number(columns = "percentage", decimals = 2) %>% 
  tab_header(title = md("CHILDCANC2: Type of Cancer Children 1-4 Diagnosed With")) %>% 
  cols_label(bodyp = md("**Cancer**"), percentage = md("**%**")) %>% 
  grand_summary_rows(columns=c(percentage),fns = ~sum(.,na.rm = T))|>
  tab_options(
      stub.font.weight = "bold"

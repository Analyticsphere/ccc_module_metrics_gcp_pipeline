...
dt_alcl= data_tib_all3  %>%   filter(D_429228540==132232896 & !is.na(D_556837046_D_141874857) & 
                                       (D_338467033==353358909 | (D_338467033==104430631 & !is.na(D_480426504)))) %>% 
  mutate(answer= case_when(!!CID_SYM==265550580 ~"Didn't Drink", 
                                                !!CID_SYM==950039557 ~"A few times per Year",
                                                !!CID_SYM==402048066 ~"A few times per Month",
                                                !!CID_SYM==522395860 ~"Once per Week",
                                                !!CID_SYM==756073829 ~"A few times per Week",
                                                !!CID_SYM==858624942 ~"Once per day",
                                                !!CID_SYM==424768954 ~"2-3 times per Day",
                                                !!CID_SYM==687445652 ~"4-5 times per Day",
                                                !!CID_SYM==333682419 ~"6+ times per Day",
                                              TRUE ~ "Skipped this question"),
         answer = factor(answer,levels=c("Didn't Drink","A few times per Year","A few times per Month","Once per Week", 
                                           "A few times per Week","Once per day","2-3 times per Day","4-5 times per Day",
                                           "6+ times per Day","Skipped this question")))
...
# Pattern logic for PRODUCT2-4----this one is having issues
funct_2_3_4 <- function(CID, CID_prev, Title){
  CID_SYM <- rlang::ensym(CID)
  CID_SYM_P <- rlang::ensym(CID_prev)

  gt_yn <- data_tib_all3 %>%
       filter((!!CID_SYM_P == 486319890) | (!!CID_SYM_P == 132232896)) %>% 
    group_by(!!CID_SYM) %>%
    summarize(n = n(), percentage = 100 * n / nrow(.)) %>%
    ungroup() %>%
    mutate(answer = dplyr::recode(!!CID_SYM, !!!dict)) %>%
...
funct_5 <- function(CID, CID_1, CID_4, Title){  
  ...
  gt_yn <- data_tib_all3 %>%
       filter(!!CID_SYM_P4 != 804785430 & (!!CID_SYM_P1==486319890 | !!CID_SYM_P1==132232896)) %>% 
    group_by(!!CID_SYM) %>%
    summarize(n = n(), percentage = 100 * n / nrow(.)) %>%
    ungroup() %>%
    mutate(answer = dplyr::recode(!!CID_SYM, !!!dict)) %>%
...
funct_8c <- function(PRODUCT8C_CID, PRODUCT5_CID, Product1_CID, Title){  
  ...
  CID_SYM_P5 <- rlang::ensym(PRODUCT5_CID)
    ...
    gt_yn <- data_tib_all3 %>%
       filter((!!CID_SYM_P1==486319890 | !!CID_SYM_P1==132232896) & !!CID_SYM_P5!=317567178) %>% 
    group_by(!!CID_SYM_P8C) %>%
    summarize(n = n(), percentage = 100 * n / nrow(.)) %>%
    ungroup() %>%
    mutate(answer = dplyr::recode(!!CID_SYM_P8C, !!!dict)) %>%
    replace_na(list(answer = "Skipped this Question")) %>%
    select(answer, n, percentage)
...
funct_9b <- function(PRODUCT1_CID, PRODUCT4_CID, PRODUCT5_CID, PRODUCT8C_CID, PRODUCT9B_CID, Title){ 
  ...
  gt_yn <- data_tib_all3 %>%
       filter((!!CID_SYM_P1==486319890 | !!CID_SYM_P1==132232896) & 
    ...
    summarize(n = n(), percentage = 100 * n / nrow(.)) %>%
    ungroup() %>%
    mutate(answer = dplyr::recode(!!CID_SYM_P9B, !!!dict)) %>%
    replace_na(list(answer = "Skipped this Question")) %>%
    select(answer, n, percentage)
...
# Pattern logic for PRODUCT10 & PRODUCT11
funct_10_11 <- function(CID, CID_prev1, CID_prev9b, CID_prev4, CID_prev8c, Title){
  ...
  CID_SYM_P8c <- rlang::ensym(CID_prev8c)
     ...
     gt_yn <- data_tib_all3 %>%
       filter((!!CID_SYM_P1 == 486319890 | !!CID_SYM_P1 == 132232896) & (!!CID_SYM_P9b == 353358909 |(!!CID_SYM_P4==804785430 | !!CID_SYM_P8c==353358909))) %>% 
    group_by(!!CID_SYM) %>%
    summarize(n = n(), percentage = 100 * n / nrow(.)) %>%
    ungroup() %>%
    mutate(answer = dplyr::recode(!!CID_SYM, !!!dict)) %>%
...
rm(list = setdiff(ls(), c("data_tib_all3", 'module3', "dict", 'hist_funct', 'boxp_funct', 'simple_funct3', 'simple_age_funct3', 'one_logic_funct3', 'alclife2_funct', 'funct_2_3_4', 'funct_5', 'funct_8c', 'funct_9b', 'funct_10_11', 'create_kable_table', 'create_kable_table_w_footnote')))
invisible(gc())
...
dt_alclH= data_tib_all3  %>%   filter(AGEGRID>70 & D_429228540==132232896 & !is.na(D_556837046_D_141874857) & 
                                       ...
                                       ((is.na(D_480426504) & as.numeric(D_556837046_D_141874857)<79) | 
                                          (!is.na(D_480426504) & as.numeric(D_556837046_D_141874857)<79 & as.numeric(D_480426504)>79)))
                                          # (as.numeric(D_480426504)>17 | (is.na(D_48042650) & (state_d_934298480==124276120 |state_d_934298480==450985724 |
                                          #                                                                                   state_d_934298480==363147933 | 
                                          #                                                                                   state_d_934298480==636706443 |state_d_934298480==771230670))) ))
...
create_kable_table(alcl4, oth_age, "ALCLIFE4A: Beverages Drank from Ages 0-17")

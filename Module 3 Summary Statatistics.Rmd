...
# Helper1: Summarizes variable responses in 'N (Pct%)' fashion
build_summary <- function(df, cid_sym) {
  df %>%
    group_by(!!cid_sym) %>%
    summarize(n = n(), .groups = "drop") %>%
    mutate(
      answer  = dplyr::recode(!!cid_sym, !!!dict),
      answer  = replace_na(answer, "Skipped this Question"),
      pct     = 100 * n / sum(n),
      `N (Pct%)` = sprintf("%s (%.1f%%)", comma(n), pct)
    ) %>%
    select(Answer = answer, `N (Pct%)`) %>%
    bind_rows(
      summarize(df, `N (Pct%)` = sprintf("%s (100.0%%)", comma(n()))) %>%
        mutate(Answer = "Total")
    )
}

# Helper2: Creates kable tables from helper1
render_kable <- function(summary_df, Title, footnote = NULL) {
  k <- kable(summary_df,
    caption = Title, align = "lr",
    booktabs = TRUE, longtable = TRUE, escape = TRUE
  ) %>%
    add_indent(seq_len(nrow(summary_df) - 1)) %>%
    kable_styling(latex_options = "scale_down")

  if (!is.null(footnote))
    k <- k %>% footnote(general = footnote, general_title = "")
  k
}
...
#1. GT table for questions asked to all (no skip logic)
simple_funct3 <- function(CID, Title){  
  cid_sym <- rlang::ensym(CID)
  render_kable(build_summary(data_tib_all3, cid_sym), Title) 
...
#2. GT table for questions asked to all 
  ## But age check required, ex: ages 50-59 or 60-69 may not apply to all
 
#Age- all participants that completed Module 3 have the derived age variable calculated
data_tib_all3$DOB <- paste0(str_sub(data_tib_all3$d_371067537,1,4), "-",
                            str_sub(data_tib_all3$d_371067537,5,6), "-",
                            str_sub(data_tib_all3$d_371067537,7,8)) 
...
simple_age_funct3 <- function(CID, Age, Title){  
  cid_sym <- rlang::ensym(CID)
  df <- filter(data_tib_all3, AGEGRID > as.numeric(Age))
  render_kable(build_summary(df, cid_sym), Title)
}
...
# Internal lookup: maps age band start to its CID and end age
  # age_start — the lower bound of the age band
alclife_bands <- list(
  "0"  = list(cid = "D_633553324_D_294629316", end = 17),
  "18" = list(cid = "D_633553324_D_164707243", end = 24),
  "25" = list(cid = "D_633553324_D_771426895", end = 29),
  "30" = list(cid = "D_633553324_D_818310825", end = 39),
  "40" = list(cid = "D_633553324_D_843593800", end = 49),
  "50" = list(cid = "D_633553324_D_175385712", end = 59),
  "60" = list(cid = "D_633553324_D_772143730", end = 69),
  "70" = list(cid = "D_633553324_D_602102163", end = 79)
)


# Levels to all ALCLIFE responses
alclife_levels <- c(
  "Didn't drink during this time", "A few times per Year", "A few times per Month",
  "Once per Week", "A few times per Week", "Once per day",
  "2-3 times per Day", "4-5 times per Day", "6+ times per Day",
  "Skipped this question"
)


# Filtering logic applied:
#   1. Base skip logic: ALCOHOL2==3 (100+ drinks lifetime), ALCOHOL4 is not NA,
#      and currently drinking (ALCOHOL5=Yes) OR stopped but have a stop age (ALCOHOL5=No & ALCOHOL6 not NA)
#   2. Age band eligibility:
#      - Participant's current age (AGEGRID) must be >= band start
#        (can't ask about a decade they haven't reached yet)
#      - Age first drank regularly (ALCOHOL4) must be <= band end
#        (must have been drinking at some point in this period)
#      - If they stopped (ALCOHOL5=No): stop age (ALCOHOL6) must be >= band start
#        (stopped at or after the band began, so they drank during at least part of this period)
#      - If still drinking (ALCOHOL5=Yes): no stop-age restriction needed
#        (already have the restriction of their current age with band)

alclife2_funct <- function(age_start, Title) {
  band <- alclife_bands[[as.character(age_start)]]
  if (is.null(band)) stop("age_start must be one of: 0, 18, 25, 30, 40, 50, 60, 70")
  ...
  df <- data_tib_all3 %>%
    dplyr::rename(
      ALCOHOL2 = D_429228540,
      ALCOHOL4 = D_556837046_D_141874857,
      ALCOHOL5 = D_338467033,
      ALCOHOL6 = D_480426504
    ) %>%
    mutate(
      ALCOHOL4 = as.numeric(ALCOHOL4),
      ALCOHOL6 = as.numeric(ALCOHOL6)
    ) %>%
    # Overall ALCLIFE skip logic
    filter(
      ALCOHOL2 == 132232896,
      !is.na(ALCOHOL4),
      ALCOHOL5 == 353358909 | (ALCOHOL5 == 104430631 & !is.na(ALCOHOL6))
    ) %>%
    # Age band eligibility filter
    filter(
      AGEGRID  >= age_start,          # participant has lived through this period (covers both current and past drinkers)
      ALCOHOL4 <= age_end,          # started drinking at or before band end (drank during this time band)
      ALCOHOL5 == 353358909 |         # still drinking, OR...
        (ALCOHOL5 == 104430631 & ALCOHOL6 >= age_start)  # stopped at or after band start (i.e. drank during this period)
    ) %>%
    mutate(
      answer = case_when(
        .data[[cid]] == 265550580 ~ "Didn't drink during this time",
        .data[[cid]] == 950039557 ~ "A few times per Year",
        .data[[cid]] == 402048066 ~ "A few times per Month",
        .data[[cid]] == 522395860 ~ "Once per Week",
        .data[[cid]] == 756073829 ~ "A few times per Week",
        .data[[cid]] == 858624942 ~ "Once per day",
        .data[[cid]] == 424768954 ~ "2-3 times per Day",
        .data[[cid]] == 687445652 ~ "4-5 times per Day",
        .data[[cid]] == 333682419 ~ "6+ times per Day",
        TRUE                      ~ "Skipped this question"
      ),
      answer = factor(answer, levels = alclife_levels)
    )

  summary_df <- df %>%
    group_by(answer) %>%
    summarize(n = n(), .groups = "drop") %>%
    mutate(`N (Pct%)` = sprintf("%s (%.1f%%)", comma(n), 100 * n / sum(n))) %>%
    select(Answer = answer, `N (Pct%)`) %>%
    bind_rows(tibble(Answer = "Total", `N (Pct%)` = sprintf("%s (100.0%%)", comma(nrow(df)))))

  render_kable(summary_df, Title)
}



#5: Pattern logic for PRODUCT2-4
funct_2_3_4 <- function(CID, CID_prev, Title){
  cid_sym   <- rlang::ensym(CID)
  cid_sym_p <- rlang::ensym(CID_prev)
  df <- filter(data_tib_all3, !!cid_sym_p %in% c(486319890, 132232896))
  render_kable(build_summary(df, cid_sym), Title)
}
...
funct_8c <- function(PRODUCT8C_CID, PRODUCT5_CID, Product1_CID, Title){  
  ...
  df <- filter(data_tib_all3,
    !!cid_sym_p1 %in% c(486319890, 132232896),
    !!cid_sym_p5 != 317567178
  )
  render_kable(build_summary(df, cid_sym_p8c), Title)
}




#8: GT TABLE: PRODUCT9B
funct_9b <- function(PRODUCT1_CID, PRODUCT4_CID, PRODUCT5_CID, PRODUCT8C_CID, PRODUCT9B_CID, Title){ 
  ...
  render_kable(build_summary(df, cid_sym_p9b), Title)
}




#9: Pattern logic for PRODUCT10 & PRODUCT11
funct_10_11 <- function(CID, CID_prev1, CID_prev9b, CID_prev4, CID_prev8c, Title){
  ...
  df <- filter(data_tib_all3,
    !!cid_sym_p1 %in% c(486319890, 132232896),
    !!cid_sym_p9b == 353358909 | !!cid_sym_p4 == 804785430 | !!cid_sym_p8c == 353358909
  )
  render_kable(build_summary(df, cid_sym), Title)
}
...
#10: General Table table
create_kable_table <- function(df, group_by_variable, Title){  
  cid_sym <- rlang::ensym(group_by_variable)
  render_kable(build_summary(df, cid_sym), Title)
}



#11: General Table table, with footnote
create_kable_table_w_footnote <- function(df, group_by_variable, Title){  
  cid_sym <- rlang::ensym(group_by_variable)
  render_kable(
    build_summary(df, cid_sym), Title,
    footnote = "Participants may select multiple responses or skip the question."
  )
}
...
data_tib_all3_CIG <- data_tib_all3 %>%  filter(D_763164658==486319890| D_763164658==132232896)
data_tib_all3_SMMAR <- data_tib_all3 %>%  filter(D_319397722==486319890| D_319397722==132232896)
data_tib_all3_tanbed3 <- data_tib_all3 %>%  filter(D_662762705==649327523| D_662762705==587052987 | D_662762705==838336013)
dttan <- data_tib_all3 %>%  filter(D_662762705!=648960871)
...
```{r AllProducts, warning=FALSE, echo=FALSE, message=FALSE}

tobacco_flags <- data_tib_all3 %>%
  mutate(
    Cigarettes = as.numeric(D_947205597_D_712653855 == 1),
    E_Cigarettes = as.numeric(D_947205597_D_706254326 == 1),
    Cigars = as.numeric(D_947205597_D_198133418 == 1),
    Smokeless_Tobacco = as.numeric(D_947205597_D_817381897 == 1),
    Hookah = as.numeric(D_947205597_D_539648641 == 1),
    Tobacco_Pipe = as.numeric(D_947205597_D_686310465 == 1),
    None = as.numeric(D_947205597_D_535003378 == 1),
    
    # Skipped = all 0 or all NA
    Skipped = if_else(rowSums(select(., starts_with("D_947205597_D_")) == 0, na.rm = FALSE) ==ncol(select(., starts_with("D_947205597_D_"))), 1, 0)
  )

tobacco_long <- tobacco_flags %>%
  pivot_longer(
    cols = c(Cigarettes, E_Cigarettes, Cigars, Smokeless_Tobacco,
             Hookah, Tobacco_Pipe, None, Skipped),
    names_to = "Product",
    values_to = "Flag"
  ) %>%
  filter(Flag == 1) %>%
  mutate(Product = case_when(
    Product == "Cigarettes" ~ "Cigarettes",
    Product == "E_Cigarettes" ~ "Electronic Cigarettes",
    Product == "Cigars" ~ "Cigars",
    Product == "Smokeless_Tobacco" ~ "Smokeless Tobacco",
    Product == "Hookah" ~ "Hookah",
    Product == "Tobacco_Pipe" ~ "Tobacco Pipe",
    Product == "None" ~ "None of the Above",
    Product == "Skipped" ~ "Skipped this Question"
  ))

create_kable_table_w_footnote(tobacco_long, Product, "Tobacco and Nicotine Products Used")
...
#CIG3.AGE
boxp_funct(D_790436165_D_408696162, "CIG3.age: Age Smoking Cigarettes on a Regular Basis", "Age")

#cig3never

data_tib_all3_CIG <- data_tib_all3 %>%  
  filter((D_763164658==486319890 | D_763164658==132232896) & is.na(D_790436165_D_408696162)) %>%  
  mutate(answer= case_when(D_790436165_D_790436165_D_648960871==1 ~ "Never smoked on a regular basis",
                                             TRUE ~ "Skipped this question"))

create_kable_table(data_tib_all3_CIG, answer, "CIG3: Smoking on a Regular Basis (Didn't Answer with Age)")
...
data_tib_all3_ECIG <- data_tib_all3 %>%  
  filter((D_624111331==486319890 | D_624111331==132232896) & is.na(D_550244580_D_517801904)) %>%  
  mutate(answer= case_when(D_550244580_D_550244580_D_648960871==1 ~ "Never smoked on a regular basis",
                           TRUE ~ "Skipped this question"))

create_kable_table_w_footnote(data_tib_all3_ECIG, answer, "ECIG3: Vaping on a Regular Basis (Didn't Answer with Age)")
...
mhgroup9= data_tib_all3 %>%  filter(D_624111331==132232896 & !is.na(D_550244580_D_517801904) & (D_751046675==804785430 | D_242276545==353358909) & 
                                      as.numeric(D_550244580_D_517801904)<19  & 
                                      (is.na(D_360277949) | !(as.numeric(D_360277949)<18))) %>% 
  mutate(surgery= case_when(D_240089798==428999623 ~ "15 minutes or less",
                            D_240089798 == 248303092 ~ "16 to 30 minutes",
                            D_240089798 == 998679771 ~ "31 to 59 minutes",
                            D_240089798 == 638092100 ~ "1 hour",
                            D_240089798 == 127455035 ~ "2 hours or more",
                            D_240089798 == 402752509 ~ "Didn’t use during this time",
                            TRUE ~ "Skipped this question"),
         surgery = factor(surgery,levels=c("15 minutes or less", "16 to 30 minutes", "31 to 59 minutes", 
                                           "1 hour", "2 hours or more","Didn’t use during this time", "Skipped this question")))

create_kable_table(mhgroup9, surgery, "ECIGLIFEA: Time E-Cigs/Vaping Devises Used per Day at Ages 0-17")


#ECIGLIFEB
mhgroup9= data_tib_all3 %>%  filter(D_624111331==132232896 & !is.na(D_550244580_D_517801904) & (D_751046675==804785430 | D_242276545==353358909) & 
                                      as.numeric(D_550244580_D_517801904)<25  & 
                                      (is.na(D_360277949) | !(as.numeric(D_360277949)<18))) %>% 
  mutate(surgery= case_when(D_536216847==428999623 ~ "15 minutes or less",
                            D_536216847 == 248303092 ~ "16 to 30 minutes",
                            D_536216847 == 998679771 ~ "31 to 59 minutes",
                            D_536216847 == 638092100 ~ "1 hour",
                            D_536216847 == 127455035 ~ "2 hours or more",
                            D_536216847 == 402752509 ~ "Didn’t use during this time",
                            TRUE ~ "Skipped this question"),
         surgery = factor(surgery,levels=c("15 minutes or less", "16 to 30 minutes", "31 to 59 minutes", 
                                           "1 hour", "2 hours or more","Didn’t use during this time", "Skipped this question")))

create_kable_table(mhgroup9, surgery, "ECIGLIFEB: Time E-Cigs/Vaping Devises Used per Day at Ages 18-24")


#ECIGLIFEC
mhgroup9= data_tib_all3 %>%  filter(D_624111331==132232896 & !is.na(D_550244580_D_517801904) & (D_751046675==804785430 | D_242276545==353358909) & 
                                      as.numeric(D_550244580_D_517801904)<30  & 
                                      (is.na(D_360277949) | !(as.numeric(D_360277949)<25))) %>% 
  mutate(surgery= case_when(D_622394149==428999623 ~ "15 minutes or less",
                            D_622394149 == 248303092 ~ "16 to 30 minutes",
                            D_622394149 == 998679771 ~ "31 to 59 minutes",
                            D_622394149 == 638092100 ~ "1 hour",
                            D_622394149 == 127455035 ~ "2 hours or more",
                            D_622394149 == 402752509 ~ "Didn’t use during this time",
                            TRUE ~ "Skipped this question"),
         surgery = factor(surgery,levels=c("15 minutes or less", "16 to 30 minutes", "31 to 59 minutes", 
                                           "1 hour", "2 hours or more","Didn’t use during this time", "Skipped this question")))

create_kable_table(mhgroup9, surgery, "ECIGLIFEC: Time E-Cigs/Vaping Devises Used per Day at Ages 25-29")


#ECIGLIFED
mhgroup9= data_tib_all3 %>%  filter(D_624111331==132232896 & !is.na(D_550244580_D_517801904) & (D_751046675==804785430 | D_242276545==353358909) & 
                                      as.numeric(D_550244580_D_517801904)<40  & 
                                      (is.na(D_360277949) | !(as.numeric(D_360277949)<30))) %>% 
...
create_kable_table(mhgroup9, surgery, "ECIGLIFED: Time E-Cigs/Vaping Devises Used per Day at Ages 30-39")
...
mhgroup9= data_tib_all3 %>%  filter(D_624111331==132232896 & !is.na(D_550244580_D_517801904) & 
                                      ...
                                      as.numeric(D_550244580_D_517801904)<50   & 
                                      (is.na(D_360277949) | !(as.numeric(D_360277949)<40)) & 
                                      as.numeric(AGEGRID)>39) %>% 
...
create_kable_table(mhgroup9, surgery, "ECIGLIFEE: Time E-Cigs/Vaping Devises Used per Day at Ages 40-49")



#ECIGLIFEF
mhgroup9= data_tib_all3 %>%  filter(D_624111331==132232896 & !is.na(D_550244580_D_517801904) & (D_751046675==804785430 | D_242276545==353358909) & 
                                      as.numeric(D_550244580_D_517801904)<60  & 
                                      (is.na(D_360277949) | !(as.numeric(D_360277949)<50)) &
                                      as.numeric(AGEGRID)>49) %>% 
...
create_kable_table(mhgroup9, surgery, "ECIGLIFEF: Time E-Cigs/Vaping Devises Used per Day at Ages 50-59")



#ECIGLIFEG
mhgroup9= data_tib_all3 %>%  filter(D_624111331==132232896 & !is.na(D_550244580_D_517801904) & (D_751046675==804785430 | D_242276545==353358909) & 
                                      as.numeric(D_550244580_D_517801904)<70 & 
                                      (is.na(D_360277949) | !(as.numeric(D_360277949)<60)) &
                                      as.numeric(AGEGRID)>59) %>% 
...
create_kable_table(mhgroup9, surgery, "ECIGLIFEG: Time E-Cigs/Vaping Devises Used per Day at Ages 60-69")
...
#ECIGLIFEH
mhgroup9= data_tib_all3 %>%  filter(D_624111331==132232896 & !is.na(D_550244580_D_517801904) & (D_751046675==804785430 | D_242276545==353358909) &
                                      as.numeric(D_550244580_D_517801904)<80 &
                                      (is.na(D_360277949) | !(as.numeric(D_360277949)<70)) &
                                      as.numeric(AGEGRID)>69) %>%
  mutate(surgery= case_when(D_866159532==428999623 ~ "15 minutes or less",
                            D_866159532 == 248303092 ~ "16 to 30 minutes",
                            D_866159532 == 998679771 ~ "31 to 59 minutes",
                            D_866159532 == 638092100 ~ "1 hour",
                            D_866159532 == 127455035 ~ "2 hours or more",
                            D_866159532 == 402752509 ~ "Didn’t use during this time",
                            TRUE ~ "Skipped this question"),
         surgery = factor(surgery,levels=c("15 minutes or less", "16 to 30 minutes", "31 to 59 minutes",
                                           "1 hour", "2 hours or more","Didn’t use during this time", "Skipped this question")))

create_kable_table(mhgroup9, surgery, "ECIGLIFEH: Time E-Cigs/Vaping Devises Used per Day at Ages 70-79")
...
#CIGAR3.AGE
boxp_funct(D_814115676_D_851145096, "CIGAR3.AGE: Age Smoking Cigars on a Regular Basis", "Age")

## Make automatic table

data_tib_all3_CIGAR <- data_tib_all3 %>%  
  filter(is.na(D_814115676_D_851145096) & (D_182431332==486319890 | D_182431332==132232896)) %>%  
  mutate(answer= case_when(D_814115676_D_814115676_D_648960871==1 ~ "Never smoked on a regular basis",
                           TRUE ~ "Skipped this question"))

create_kable_table(data_tib_all3_CIGAR, answer, "CIGAR3: Smoking Cigars on a Regular Basis (Didn't Answer with Age)")
...
cigar_kind = data_tib_all3 %>%  
  filter((D_182431332==486319890 | D_182431332==132232896) & (D_287715042==353358909 | (D_564438246==804785430 | D_395480458==353358909))) %>% 
...
create_kable_table(cigar_kind, kind, "CIGAR10: Kind of Cigar Smoked")
...
#CHEW3
boxp_funct(D_257300783_D_424527685, "CHEW3.AGE: Age at Using Chewing Tobacco/Dip on a Regular Basis", "Age")


#CHEW_NEVER
data_tib_all3_CHEW <- data_tib_all3 %>%  
  filter((D_306320432==486319890 | D_306320432==132232896) & is.na(D_257300783_D_424527685)) %>% 
  mutate(answer= case_when(D_257300783_D_257300783_D_648960871==1 ~ "Never smoked on a regular basis",
                           TRUE ~ "Skipped this question"))

create_kable_table(data_tib_all3_CHEW, answer, "CHEW3: Using Chewing Tobacco on a Regular Basis (Didn't Answer with Age)")
...
lifetime= data_tib_all3 %>%  filter(D_306320432==132232896 & !is.na(D_257300783_D_424527685) & 
                                      ...
                                      as.numeric(D_257300783_D_424527685)<18 & 
                                      (is.na(D_671380198) | !(as.numeric(D_671380198)<18))) %>% 
  mutate(timespan= case_when(D_495592561==428999623 ~ "15 minutes or less",
                             ...
                             D_495592561==402752509 ~ "Didn’t use during this time",
                                             TRUE ~ "Skipped this question"),
         timespan = factor(timespan,levels=c("15 minutes or less", "16 to 30 minutes", "31 to 59 minutes", "1 hour", 
                                             "2 hours or more","Didn’t use during this time", "Skipped this question")))

create_kable_table(lifetime, timespan, "CHEWLIFEA: Time Chewing Tobacco/Dip Used per Day at Ages 0-17")
...
lifetime= data_tib_all3 %>%  filter(D_306320432==132232896 & !is.na(D_257300783_D_424527685) & 
                                      (D_101710639==804785430 | D_785139115==353358909) & 
                                      as.numeric(D_257300783_D_424527685)<25 & 
                                      (is.na(D_671380198) | !(as.numeric(D_671380198)<18))) %>% 
...
create_kable_table(lifetime, timespan, "CHEWLIFEB: Time Chewing Tobacco/Dip Used per Day at Ages 18-24")
...
lifetime= data_tib_all3 %>%  filter(D_306320432==132232896 & !is.na(D_257300783_D_424527685) & 
                                      (D_101710639==804785430 | D_785139115==353358909) & 
                                      as.numeric(D_257300783_D_424527685)<30 & 
                                      (is.na(D_671380198) | !(as.numeric(D_671380198)<25))) %>% 
...
create_kable_table(lifetime, timespan, "CHEWLIFEC: Time Chewing Tobacco/Dip Used per Day at Ages 25-29")
...
lifetime= data_tib_all3 %>%  filter(D_306320432==132232896 & !is.na(D_257300783_D_424527685) & 
                                      (D_101710639==804785430 | D_785139115==353358909) & 
                                      as.numeric(D_257300783_D_424527685)<40 & 
                                      (is.na(D_671380198) | !(as.numeric(D_671380198)<30))) %>% 
...
create_kable_table(lifetime, timespan, "CHEWLIFED: Time Chewing Tobacco/Dip Used per Day at Ages 30-39")


#CHEWLIFEE
lifetime= data_tib_all3 %>%  filter(D_306320432==132232896 & !is.na(D_257300783_D_424527685) & 
                                      (D_101710639==804785430 | D_785139115==353358909) & 
                                      as.numeric(D_257300783_D_424527685)<50 & 
                                      (is.na(D_671380198) | !(as.numeric(D_671380198)<40))) %>% 
...
create_kable_table(lifetime, timespan, "CHEWLIFEE: Time Chewing Tobacco/Dip Used per Day at Ages 40-49")
...
lifetime= data_tib_all3 %>%  filter(D_306320432==132232896 & !is.na(D_257300783_D_424527685) & 
                                      (D_101710639==804785430 | D_785139115==353358909) & 
                                      as.numeric(D_257300783_D_424527685)<60 & 
                                      (is.na(D_671380198) | !(as.numeric(D_671380198)<50)) &
                                      as.numeric(AGEGRID)>49) %>% 
...
create_kable_table(lifetime, timespan, "CHEWLIFEF: Time Chewing Tobacco/Dip Used per Day at Ages 50-59")
...
lifetime= data_tib_all3 %>%  filter(D_306320432==132232896 & !is.na(D_257300783_D_424527685) & 
                                      (D_101710639==804785430 | D_785139115==353358909) & 
                                      as.numeric(D_257300783_D_424527685)<70 & 
                                      (is.na(D_671380198) | !(as.numeric(D_671380198)<60)) & 
                                      as.numeric(AGEGRID)>59) %>% 
...
create_kable_table(lifetime, timespan, "CHEWLIFEG: Time Chewing Tobacco/Dip Used per Day at Ages 60-69")
...
lifetime= data_tib_all3 %>%  filter(D_306320432==132232896 & !is.na(D_257300783_D_424527685) & 
                                      ...
                                      (is.na(D_671380198) | !(as.numeric(D_671380198)<60)) & 
                                      as.numeric(AGEGRID)>59) %>% 
  mutate(timespan= case_when(D_551301510==428999623 ~ "15 minutes or less",
                             D_551301510 == 248303092 ~ "16 to 30 minutes",
                             D_551301510 == 998679771 ~ "31 to 59 minutes",
                             D_551301510 == 638092100 ~ "1 hour",
                             D_551301510 == 127455035 ~ "2 hours or more",
                             D_551301510 == 402752509 ~ "Didn’t use during this time",
                             TRUE ~ "Skipped this question"),
         timespan = factor(timespan,levels=c("15 minutes or less", "16 to 30 minutes", "31 to 59 minutes", "1 hour", 
                                                      "2 hours or more","Didn’t use during this time", "Skipped this question")))

create_kable_table(lifetime, timespan, "CHEWLIFEH: Time Chewing Tobacco/Dip Used per Day at Ages 70+")
...
```{r Clear_Memory0, include=FALSE}
#Clearing up some memory -- removing all but the main dataframe and functions
rm(list = setdiff(ls(), c("data_tib_all3", 'module3', "dict", 'hist_funct', 'boxp_funct', 'simple_funct3', 'simple_age_funct3', 'one_logic_funct3', 'alclife2_funct', "alclife_bands", "alclife_levels", 'funct_2_3_4', 'funct_5', 'funct_8c', 'funct_9b', 'funct_10_11', 'create_kable_table', 'create_kable_table_w_footnote',"render_kable", "build_summary")))
invisible(gc())
...
#HOOKAH3.AGE
boxp_funct(D_154603497_D_234430294, "HOOKAH3.AGE: Age Smoking Hookah on a Regular Basis", "Age")

#HOOKAH3 (never)
data_tib_all3_HOOKAH <- data_tib_all3 %>%  
  filter((D_981755099==486319890| D_981755099==132232896) & is.na(D_154603497_D_234430294)) %>% 
  mutate(answer= case_when(D_154603497_D_154603497_D_648960871==1 ~ "Never smoked on a regular basis",
                           TRUE ~ "Skipped this question"))

create_kable_table(data_tib_all3_HOOKAH, answer, "HOOKAH3: Smoking Hookah on a Regular Basis (Didn't Answer with Age)")
...
#PIPE3.AGE
boxp_funct(D_546410473_D_663551714, "PIPE3.AGE: Age Smoking Tobacco Pipes on a Regular Basis", "Age")


#Make automated table
data_tib_all3_PIPE <- data_tib_all3 %>%  
  filter((D_559388168==486319890| D_559388168==132232896) & is.na(D_546410473_D_663551714)) %>% 
  mutate(answer= case_when(D_546410473_D_546410473_D_648960871==1 ~ "Never smoked on a regular basis",
                           TRUE ~ "Skipped this question"))

create_kable_table(data_tib_all3_PIPE, answer, "PIPE3: Smoking Tobacco Pipes on a Regular Basis (Didn't Answer with Age)")
...
```{r AllMairjuana, warning=FALSE, echo=FALSE, message=FALSE}

MJ_pd <- data_tib_all3 %>% 
  mutate(
    Smoking = as.numeric(D_830608495_D_760521319 == 1),
    Smoking_with_Tobacco = as.numeric(D_830608495_D_158841370 == 1),
    Vaping = as.numeric(D_830608495_D_349034127 == 1),
    Edible = as.numeric(D_830608495_D_713594719 == 1),
    Dabbing = as.numeric(D_830608495_D_675129470 == 1),
    None = as.numeric(D_830608495_D_535003378 == 1),
    Skipped = if_else(rowSums(select(., starts_with("D_830608495_D_")) == 0, na.rm = FALSE) ==ncol(select(., starts_with("D_830608495_D_"))), 1, 0)
  )
...
MJ_long <- MJ_pd %>%
  pivot_longer(
    cols = c(Smoking, Smoking_with_Tobacco, Vaping,
             Edible, Dabbing, None, Skipped),
    names_to = "Product",
    values_to = "Flag"
  ) %>%
  filter(Flag == 1) %>%
  mutate(Product = case_when(
    ...
    Product == "Smoking_with_Tobacco" ~ "Smoking combined with Tobacco",
    Product == "Vaping" ~ "Electronic Vaping",
    Product == "Edible" ~ "Consuming through Food or Drink",
    Product == "Dabbing" ~ "THC Dabbing",
    Product == "None" ~ "None of the Above",
    Product == "Skipped" ~ "Skipped this Question"
  ))

create_kable_table_w_footnote(MJ_long, Product, "Method of Consuming Marijuana")
...
#SMMAR3.AGE
boxp_funct(D_116080663_D_889949861, "SMMAR3.AGE: Age Smoking Marijuana on a Regular Basis", "Age")


#Make automated table
data_tib_all3_SMMAR <- data_tib_all3 %>%  
  filter((D_319397722==486319890| D_319397722==132232896) & is.na(D_116080663_D_889949861)) %>%  
  mutate(answer= case_when(D_116080663_D_116080663_D_648960871==1 ~ "Never smoked on a regular basis",
                           TRUE ~ "Skipped this question"))

create_kable_table(data_tib_all3_SMMAR, answer, "SMMAR3: Smoking Marijuana on a Regular Basis (Didn't Answer with Age)")
...
lifetime= data_tib_all3 %>%  filter(D_319397722==132232896 & !is.na(D_116080663_D_889949861) & (D_503058092==804785430 | D_129677435==353358909) & 
                                      as.numeric(D_116080663_D_889949861)<19  & 
                                      (is.na(D_863664828) | !(as.numeric(D_863664828)<17))) %>% 
...
create_kable_table(lifetime, timespan, "SMMARLIFEA: Time Spent per Day Smoking Marijuana Ages 0-17")
...
lifetime= data_tib_all3 %>%  filter(D_319397722==132232896 & !is.na(D_116080663_D_889949861) & 
                                      ...
                                      as.numeric(D_116080663_D_889949861)<24  & 
                                      (is.na(D_863664828) | !(as.numeric(D_863664828)<18))) %>% 
...
create_kable_table(lifetime, timespan, "SMMARLIFEB: Time Spent per Day Smoking Marijuana Ages 18-24")
...
lifetime= data_tib_all3 %>%  filter(D_319397722==132232896 & !is.na(D_116080663_D_889949861) & 
                                      (D_503058092==804785430 | D_129677435==353358909) & 
                                      as.numeric(D_116080663_D_889949861)<30  & 
                                      (is.na(D_863664828) | !(as.numeric(D_863664828)<25))) %>% 
...
create_kable_table(lifetime, timespan, "SMMARLIFEC: Time Spent per Day Smoking Marijuana Ages 25-29")
...
lifetime= data_tib_all3 %>%  filter(D_319397722==132232896 & !is.na(D_116080663_D_889949861) & 
                                      (D_503058092==804785430 | D_129677435==353358909) & 
                                      as.numeric(D_116080663_D_889949861)<40  & 
                                      (is.na(D_863664828) | !(as.numeric(D_863664828)<30))) %>% 
...
create_kable_table(lifetime, timespan, "SMMARLIFED: Time Spent per Day Smoking Marijuana Ages 30-39")
...
lifetime= data_tib_all3 %>%  filter(D_319397722==132232896 & !is.na(D_116080663_D_889949861) & 
                                      (D_503058092==804785430 | D_129677435==353358909) & 
                                      as.numeric(D_116080663_D_889949861)<50  & 
                                      (is.na(D_863664828) | !(as.numeric(D_863664828)<40)) &
                                      as.numeric(AGEGRID)>39) %>% 
...
create_kable_table(lifetime, timespan, "SMMARLIFEE: Time Spent per Day Smoking Marijuana Ages 40-49")


#SMMARLIFEF
lifetime= data_tib_all3 %>%  filter(D_319397722==132232896 & !is.na(D_116080663_D_889949861) & 
                                      (D_503058092==804785430 | D_129677435==353358909) & 
                                      as.numeric(D_116080663_D_889949861)<60  & 
                                      (is.na(D_863664828) | !(as.numeric(D_863664828)<50)) & 
                                      as.numeric(AGEGRID)>49) %>% 
...
create_kable_table(lifetime, timespan, "SMMARLIFEF: Time Spent per Day Smoking Marijuana Ages 50-59")
...
lifetime= data_tib_all3 %>%  filter(D_319397722==132232896 & !is.na(D_116080663_D_889949861) & 
                                      (D_503058092==804785430 | D_129677435==353358909) & 
                                      as.numeric(D_116080663_D_889949861)<70 & 
                                      (is.na(D_863664828) | !(as.numeric(D_863664828)<60)) &
                                      as.numeric(AGEGRID)>59 ) %>%
...
create_kable_table(lifetime, timespan, "SMMARLIFEG: Time Spent per Day Smoking Marijuana Ages 60-69")

#SMMARLIFEH
lifetime= data_tib_all3 %>%  filter(D_319397722==132232896 & !is.na(D_116080663_D_889949861) & 
                                      (D_503058092==804785430 | D_129677435==353358909) & 
                                      as.numeric(D_116080663_D_889949861)<80 & 
                                      (is.na(D_863664828) | !(as.numeric(D_863664828)<70)) &
                                      as.numeric(AGEGRID)>69 ) %>%
  mutate(timespan= case_when(D_882838859==428999623 ~ "15 minutes or less",
                             D_882838859 == 248303092 ~ "16 to 30 minutes",
                             D_882838859 == 998679771 ~ "31 to 59 minutes",
                             D_882838859 == 638092100 ~ "1 hour",
                             D_882838859 == 127455035 ~ "2 hours or more",
                             D_882838859 == 402752509 ~ "Didn’t use during this time",
                             TRUE ~ "Skipped this question"),
         timespan = factor(timespan,levels=c("15 minutes or less", "16 to 30 minutes", "31 to 59 minutes", "1 hour", 
                                                       "2 hours or more","Didn’t use during this time", "Skipped this question")))

create_kable_table(lifetime, timespan, "SMMARLIFEH: Time Spent per Day Smoking Marijuana Ages 70+") 
...
#TOMAR1
one_logic_funct3(D_524914900,D_830608495_D_158841370 ,1,"TOMAR1: Total Majiuana and Tobacco Combined Products Smoked in a Lifetime")


#TOMAR2
boxp_funct(D_140848765, "TOMAR2: Age First Smoked a Majiuana and Tobacco Combined Product", "Age")


#TOMAR3
boxp_funct(D_378254382_D_628818510, "TOMAR3: Age Smoking Majiuana and Tobacco Combined Products \n on a Regular Basis", "Age")


#Make automated table
data_tib_all3_TOMAR <- data_tib_all3 %>%  
  filter((D_524914900==486319890| D_524914900==132232896) & is.na(D_378254382_D_628818510)) %>%  
  mutate(answer= case_when(D_378254382_D_378254382_D_648960871==1 ~ "Never smoked on a regular basis",
                           TRUE ~ "Skipped this question"))

create_kable_table(data_tib_all3_TOMAR, answer, "TOMAR3: Smoking Marijuana Combined with Tobacco on a Regular Basis (Didn't Answer with Age)")



#TOMAR4
funct_2_3_4(D_891825638,D_524914900,"TOMAR4: Smoking Majiuana and Tobacco Combined Products Now")

#TOMAR5
funct_5(D_444759994,D_524914900,D_891825638,"TOMAR5: Last Time Majiuana and Tobacco Combined Products Smoked")


#TOMAR6
tomr6=hist_funct(D_746817928, "TOMAR6: Majiuana and Tobacco Combined Products Smoked \n in the past 30 days", "Products", "Frequency")

#TOMAR7A
tomr7=hist_funct(D_992757226, "TOMAR7: Months Ago Majiuana and Tobacco Combined Products \n Last Smoked", "Months", "Frequency")
grid.arrange(tomr6, tomr7, nrow=2)


#TOMAR8
boxp_funct(D_405178123, "TOMAR8: Age at Last Majiuana and Tobacco Combined Product", "Age")
...
#TOMAR8D
hist_funct(D_363377579, "TOMAR8D: Days per Month Last Majiuana and Tobacco \n Combined Products Smoked", "Days", "Frequency")

#TOMAR9A
hist_funct(D_964154290, "TOMAR9A: Majiuana and Tobacco Combined Products Smoked per Day", "Products", "Frequency")
...
#TOMAR9D
boxp_funct(D_606675249, "TOMAR9D: Age You Stopped Smoking Majiuana and Tobacco \n Combined Products Every Day", "Age")

#TOMAR9C
hist_funct(D_411960217, "TOMAR9C: Times per Day Majiuana and Tobacco \n Combined Products Smoked", "Times", "Frequency")
...
#TOMARLIFEA
#hist_funct(D_381268397, "TOMARLIFEA: Smoking Majiuana and Tobacco Combined Products: Ages 0-17", "Times per Day", "Frequency") #no one yet 


#TOMARLIFEB
tomrlfB=hist_funct(D_723763329, "TOMARLIFEB: Smoking Majiuana and Tobacco Combined Products: \n Ages 18-24", "Times per Day", "Frequency")


#TOMARLIFEC
tomrlfC=hist_funct(D_724990284, "TOMARLIFEC: Smoking Majiuana and Tobacco Combined Products: \n Ages 25-29", "Times per Day", "Frequency")
grid.arrange(tomrlfB, tomrlfC, nrow=2)


#TOMARLIFED
tomrlfD=hist_funct(D_383748300, "TOMARLIFED: Smoking Majiuana and Tobacco Combined Products: \n Ages 30-39", "Times per Day", "Frequency")


#TOMARLIFEE
tomrlfE=hist_funct(D_199612020, "TOMARLIFEE: Smoking Majiuana and Tobacco Combined Products: \n Ages 40-49", "Times per Day", "Frequency")
grid.arrange(tomrlfD, tomrlfE, nrow=2)


#TOMARLIFEF
#hist_funct(D_755298453, "TOMARLIFEF: Smoking Majiuana and Tobacco Combined Products: \n Ages 50-59", "Times per Day", "Frequency") #no one yet 


#TOMARLIFEG
#hist_funct(D_777877159, "TOMARLIFEG: Smoking Majiuana and Tobacco Combined Products: \n Ages 60-69", "Times per Day", "Frequency") #no one yet 


#TOMARLIFEH
#hist_funct(D_680697711, "TOMARLIFEH: Smoking Majiuana and Tobacco Combined Products: \n Ages 70+", "Times per Day", "Frequency")#no one yet 

```



```{r Clear_Memory1.5, include=FALSE}
#Clearing up some memory -- removing all but the main dataframe and functions
rm(list = setdiff(ls(), c("data_tib_all3", 'module3', "dict", 'hist_funct', 'boxp_funct', 'simple_funct3', 'simple_age_funct3', 'one_logic_funct3', 'alclife2_funct', "alclife_bands", "alclife_levels", 'funct_2_3_4', 'funct_5', 'funct_8c', 'funct_9b', 'funct_10_11', 'create_kable_table', 'create_kable_table_w_footnote',"render_kable", "build_summary")))
invisible(gc())
...
#ELMAR3
boxp_funct(D_942945860_D_742716427, "ELMAR3.AGE: Age Vaping Marijuana on a Regular Basis", "Age")


#Make automated table
data_tib_all3_ELMAR <- data_tib_all3 %>%  
  filter((D_325229459==486319890| D_325229459==132232896) & is.na(D_942945860_D_742716427)) %>%  
  mutate(answer= case_when(D_942945860_D_942945860_D_648960871==1 ~ "Never smoked on a regular basis",
                           TRUE ~ "Skipped this question"))

create_kable_table(data_tib_all3_ELMAR, answer, "ELMAR3: Vaping Marijuana on a Regular Basis (Didn't Answer Age)")
...
lifetime= data_tib_all3 %>%  filter(D_325229459==132232896 & !is.na(D_942945860_D_742716427) & 
                                      ...
                                      as.numeric(D_942945860_D_742716427)<19  & 
                                      (is.na(D_445187084) | !(as.numeric(D_445187084)<18))) %>% 
  mutate(timespan= case_when(D_446121108==428999623 ~ "15 minutes or less",
                             D_446121108 == 248303092 ~ "16 to 30 minutes",
                             D_446121108 == 998679771 ~ "31 to 59 minutes",
                             D_446121108 == 638092100 ~ "1 hour",
                             D_446121108 == 127455035 ~ "2 hours or more",
                             D_446121108 == 402752509 ~ "Didn’t use during this time",
                             TRUE ~ "Skipped this question"),
         timespan = factor(timespan,levels=c("15 minutes or less", "16 to 30 minutes", "31 to 59 minutes", "1 hour", 
                                             "2 hours or more","Didn’t use during this time", "Skipped this question")))

create_kable_table(lifetime, timespan, "ELMARLIFEA: Time Spent per Day Vaping Marijuana: Ages 0-17")


#ELMARLIFEB
lifetime= data_tib_all3 %>%  filter(D_325229459==132232896 & !is.na(D_942945860_D_742716427) & 
                                      (D_521073718==804785430 | D_142164107==353358909) & 
                                      as.numeric(D_942945860_D_742716427)<25  & 
                                      (is.na(D_445187084) | !(as.numeric(D_445187084)<18))) %>% 
  mutate(timespan= case_when(D_822964046==428999623 ~ "15 minutes or less",
                             D_822964046 == 248303092 ~ "16 to 30 minutes",
                             D_822964046 == 998679771 ~ "31 to 59 minutes",
                             D_822964046 == 638092100 ~ "1 hour",
                             D_822964046 == 127455035 ~ "2 hours or more",
                             D_822964046 == 402752509 ~ "Didn’t use during this time",
                             TRUE ~ "Skipped this question"),
         timespan = factor(timespan,levels=c("15 minutes or less", "16 to 30 minutes", "31 to 59 minutes", "1 hour", 
                                             "2 hours or more","Didn’t use during this time", "Skipped this question")))

create_kable_table(lifetime, timespan, "ELMARLIFEB: Time Spent per Day Vaping Marijuana: Ages 18-24")


#ELMARLIFEC
lifetime= data_tib_all3 %>%  filter(D_325229459==132232896 & !is.na(D_942945860_D_742716427) & 
                                      (D_521073718==804785430 | D_142164107==353358909) & 
                                      as.numeric(D_942945860_D_742716427)<30  & 
                                      (is.na(D_445187084) | !(as.numeric(D_445187084)<25))) %>% 
  mutate(timespan= case_when(D_822964046==428999623 ~ "15 minutes or less",
                             D_822964046 == 248303092 ~ "16 to 30 minutes",
                             D_822964046 == 998679771 ~ "31 to 59 minutes",
                             D_822964046 == 638092100 ~ "1 hour",
...
create_kable_table(lifetime, timespan, "ELMARLIFEC: Time Spent per Day Vaping Marijuana: Ages 25-29")


#ELMARLIFED
lifetime= data_tib_all3 %>%  filter(D_325229459==132232896 & !is.na(D_942945860_D_742716427) & 
                                      (D_521073718==804785430 | D_142164107==353358909) & 
                                      as.numeric(D_942945860_D_742716427)<40  & 
                                      (is.na(D_445187084) | !(as.numeric(D_445187084)<30))) %>% 
...
create_kable_table(lifetime, timespan, "ELMARLIFED: Time Spent per Day Vaping Marijuana: Ages 30-39")
...
lifetime= data_tib_all3 %>%  filter(D_325229459==132232896 & !is.na(D_942945860_D_742716427) & 
                                      (D_521073718==804785430 | D_142164107==353358909) & 
                                      as.numeric(D_942945860_D_742716427)<50  & 
                                      (is.na(D_445187084) | !(as.numeric(D_445187084)<40)) & 
                                      as.numeric(AGEGRID)>39) %>% 
...
create_kable_table(lifetime, timespan, "ELMARLIFEE: Time Spent per Day Vaping Marijuana: Ages 40-49")
...
lifetime= data_tib_all3 %>%  filter(D_325229459==132232896 & !is.na(D_942945860_D_742716427) & 
                                      (D_521073718==804785430 | D_142164107==353358909) & 
                                      as.numeric(D_942945860_D_742716427)<60  & 
                                      (is.na(D_445187084) | !(as.numeric(D_445187084)<50)) & 
                                      as.numeric(AGEGRID)>49) %>% 
...
create_kable_table(lifetime, timespan, "ELMARLIFEF: Time Spent per Day Vaping Marijuana: Ages 50-59")
...
lifetime= data_tib_all3 %>%  filter(D_325229459==132232896 & !is.na(D_942945860_D_742716427) & 
                                      (D_521073718==804785430 | D_142164107==353358909) & 
                                      as.numeric(D_942945860_D_742716427)<70  & 
                                      (is.na(D_445187084) | !(as.numeric(D_445187084)<60)) & 
                                      as.numeric(AGEGRID)>59) %>% 
...
create_kable_table(lifetime, timespan, "ELMARLIFEG: Time Spent per Day Vaping Marijuana: Ages 60-69")
...
lifetime= data_tib_all3 %>%  filter(D_325229459==132232896 & !is.na(D_942945860_D_742716427) & 
                                      (D_521073718==804785430 | D_142164107==353358909) & 
                                      as.numeric(D_942945860_D_742716427)<80  & 
                                      (is.na(D_445187084) | !(as.numeric(D_445187084)<70)) & 
                                      as.numeric(AGEGRID)>69) %>% 
  mutate(timespan= case_when(D_586306068==428999623 ~ "15 minutes or less",
                             D_586306068 == 248303092 ~ "16 to 30 minutes",
                             D_586306068 == 998679771 ~ "31 to 59 minutes",
                             D_586306068 == 638092100 ~ "1 hour",
                             D_586306068 == 127455035 ~ "2 hours or more",
                             D_586306068 == 402752509 ~ "Didn’t use during this time",
                             TRUE ~ "Skipped this question"),
         timespan = factor(timespan,levels=c("15 minutes or less", "16 to 30 minutes", "31 to 59 minutes", "1 hour", 
                                             "2 hours or more","Didn’t use during this time", "Skipped this question")))

create_kable_table(lifetime, timespan, "ELMARLIFEH: Time Spent per Day Vaping Marijuana: Ages 70+")
...
#EAMAR3
boxp_funct(D_653198444_D_622843252, "EAMAR3.AGE: Age Consuming Marijuana in Food/Drinks on a Regular Basis", "Age")


#Make automated table
EAMAR3 <- data_tib_all3 %>%  
  filter((D_101170268==486319890| D_101170268==132232896) & is.na(D_653198444_D_622843252)) %>% 
  mutate(answer= case_when(D_653198444_D_653198444_D_648960871==1 ~ "Never smoked on a regular basis",
                           TRUE ~ "Skipped this question"))

create_kable_table(EAMAR3, answer, "EAMAR3: Consuming Marijuana on a Regular Basis (Didn't Answer with Age)")
...
EAMAR9A_ans= data_tib_all3 %>%  
  filter((data_tib_all3$D_101170268==486319890| data_tib_all3$D_101170268==132232896) & is.na(data_tib_all3$D_653198444_D_622843252))  %>%  
  mutate(answer= case_when(D_276575533_D_276575533_D_178420302=="0" ~ "Answered with Number of Servings",
                           D_276575533_D_276575533_D_178420302=="1" ~ "Answered Don't Know",
                           TRUE ~ "Skipped this question"))

create_kable_table(EAMAR9A_ans, answer, "EAMAR9A: Don't Know Servings of Marijuana")
...
#THMAR3
boxp_funct(D_742254559_D_185168695, "THMAR3.AGE: Age at First THC Dabbing Usage", "Age")


#Make automated table
THMAR3 <- data_tib_all3 %>%  
  filter((D_355689185==486319890 | D_355689185==132232896) & is.na(D_742254559_D_185168695)) %>% 
  mutate(answer= case_when(D_742254559_D_742254559_D_648960871=="1" ~ "Never smoked on a regular basis",
                           TRUE ~ "Skipped this question"))

create_kable_table(THMAR3, answer, "THMAR3: THC Dabbing on a Regular Basis (Didn't Answer with Age)")
...
THMAR9A_ans= data_tib_all3 %>%  
  filter((data_tib_all3$D_355689185==486319890| data_tib_all3$D_355689185==132232896) & is.na(data_tib_all3$D_742254559_D_185168695)) %>%  
  mutate(answer= case_when(D_933417196_D_933417196_D_178420302=="0" ~ "Answered with Number of Servings",
                           D_933417196_D_933417196_D_178420302=="1" ~ "Answered Don't Know",
                           TRUE ~ "Skipped this question"))

create_kable_table(THMAR9A_ans, answer, "THMAR9A: Don't Know Servings of Marijuana")
...
#ALCOHOL3
boxp_funct(D_250456537, "ALCOHOL3.AGE: Age at First Alcoholic Beverage", "Age")

#ALCOHOL4
boxp_funct(D_556837046_D_141874857, "ALCOHOL4_AGE: Age Drinking Alcohol on a Regular Basis", "Age")
  #Alcohol4_never
data_tib_all3_ALC4 <- data_tib_all3 %>%  
  filter((D_429228540==486319890 | D_429228540==132232896) & is.na(D_556837046_D_141874857)) %>%  
  mutate(answer= case_when(D_556837046_D_556837046_D_648960871==1 ~ "Never drank on a regular basis",
                           TRUE ~ "Skipped this question"))

create_kable_table(data_tib_all3_ALC4, answer, "ALCOHOL4: Drinking on a Regular Basis (Didn't Answer with Age)")


#ALCOHOL5
dt_ALC5= data_tib_all3  %>% filter(D_429228540==486319890 | D_429228540==132232896) %>%  
  mutate(answer= case_when(D_338467033==104430631 ~ "No",
                                             D_338467033==353358909 ~ "Yes",
                                             is.na(D_338467033) ~ "Skipped this question"))

create_kable_table(dt_ALC5, answer, "ALCOHOL5: Consumed an Alcoholic Beverage in the Last 12 Months")
...
dtALC7= data_tib_all3 %>% filter(D_448968121==353358909 & (D_338467033==104430631 | is.na(D_338467033))) %>%  
  mutate(answers= case_when(D_789159346_D_789159346_D_895826652 ==1 ~ "Personal Choice",
...
create_kable_table(dtALC7, answers, "ALCOHOL7: Reason You stopped Drinking")


```


```{r Alcohol8, warning=FALSE, echo=FALSE, message=FALSE}


alc_flags <-  data_tib_all3 %>%
  filter(D_338467033 == 353358909) %>% 
  mutate(
    Beer = as.numeric(D_447720598_D_549079588 == 1),
    Liquor = as.numeric(D_447720598_D_896953195 == 1),
    Wine = as.numeric(D_447720598_D_889023234 == 1),
    Other = as.numeric(D_447720598_D_181769837 == 1),
    Skipped = if_else(rowSums(select(., starts_with("D_447720598_D_")) == 0, na.rm = FALSE) ==ncol(select(., starts_with("D_447720598_D_"))), 1, 0)
  )

  
alcohol_long <- alc_flags %>%
  pivot_longer(
    cols = c(Beer, Liquor, Wine, Other, Skipped),
    names_to = "Drink_Type",
    values_to = "Flag"
  ) %>%
  filter(Flag == 1) %>%
  mutate(Drink_Type = case_when(
    Drink_Type == "Beer" ~ "Beer",
    Drink_Type == "Liquor" ~ "Liquor",
    Drink_Type == "Wine" ~ "Wine",
    Drink_Type == "Other" ~ "Other",
    Drink_Type == "Skipped" ~ "Skipped this Question"
  ))

create_kable_table_w_footnote(alcohol_long, Drink_Type, "ALCOHOL8: Beverages Drank over the Last 12 Months")
...
alc9 <- data_tib_all3 %>%  filter(D_429228540==132232896 & !is.na(D_556837046_D_141874857) & D_338467033==353358909) %>%  
  mutate(answer= case_when(D_254739443==648960871 ~ "Never",
                            ...
                            TRUE ~ "Skipped This Question"),
         answer = factor(answer,levels=c("Never", "1 to 5 times per year", "6 to 11 times per year","Once per month",
                                                "2 to 3 times per month","Once per week","Twice per week",
                                                "3 to 4 times per week", "5 to 6 times per week", "Every Day",
                                                "Skipped This Question")))

create_kable_table(alc9, answer, "ALCOHOL9: Frequency of Drinking 4-5 Alcoholic Beverages in One Sitting/Ocassion")

```


```{r Clear_Memory2.5, include=FALSE}
#Clearing up some memory -- removing all but the main dataframe and functions
rm(list = setdiff(ls(), c("data_tib_all3", 'module3', "dict", 'hist_funct', 'boxp_funct', 'simple_funct3', 'simple_age_funct3', 'one_logic_funct3', 'alclife2_funct', "alclife_bands", "alclife_levels", 'funct_2_3_4', 'funct_5', 'funct_8c', 'funct_9b', 'funct_10_11', 'create_kable_table', 'create_kable_table_w_footnote', "render_kable", "build_summary")))
invisible(gc())
...
#----------------------- THIS IS OLD CODE I'M LOOKING TO REPLACE ----------------------
...
# ------ Skip logic to see the question set* ------------------------
# ALC2=3, ALC4 IS NOT NA, & (ALC5=1 OR (ALC5=0 & ALC6 IS NOT NA)) 

dt_alcl= data_tib_all3  %>% 
  dplyr::rename("ALCOHOL2"="D_429228540",
                "ALCOHOL4"="D_556837046_D_141874857",
                "ALCOHOL5"="D_338467033",
                "ALCOHOL6"="D_480426504") %>% 
  filter(ALCOHOL2==132232896 & !is.na(ALCOHOL4) & 
           (ALCOHOL5==353358909 | (ALCOHOL5==104430631 & !is.na(ALCOHOL6))))  %>% 
  mutate(ALCOHOL4 = as.numeric(ALCOHOL4),
         ALCOHOL6 = as.numeric(ALCOHOL6))

# ----- Additional skip logic in regards to age range ---------------


#1: Starting age range can't be greater tan their current age
  # Ex: someone 55 can't be asked about their drinking from ages 70-79
  # ALCLIFE_START =< AGEGRID

#2: IF THEY NEVER STOPPED DRINKING:
  #THEN AGE STARTED (ALCOHOL4) =<ALCLIFE_START

#3: IF THEY DID STOP:
  #THEN STARTED (ALCOHOL4) =<ALCLIFE_START AND ALCLIFE_END=< AGE STOPPED (ALCOHOL6) :


# COMBINED:
  #ALC4=<ALCLIFEAGE_START AND AGEGRID >= ALCLIFE_START AND
  #EITHER ALCOHOL5=YES OR (ALCOHOL5==NO THEN ALCLIFE_END =< ALCOHOL6)
...
#ALCLIFE2A 
dt_alclA= data_tib_all3  %>%   filter(D_429228540==132232896 & !is.na(D_556837046_D_141874857) & 
                                       (D_338467033==353358909 | (D_338467033==104430631 & !is.na(D_480426504))) & 
                                       ((is.na(D_480426504) & as.numeric(D_556837046_D_141874857)<17) | 
                                          (!is.na(D_480426504) & as.numeric(D_556837046_D_141874857)<17 & as.numeric(D_480426504)>17)))
                                          # (as.numeric(D_480426504)>17 | (is.na(D_48042650) & (state_d_934298480==124276120 |state_d_934298480==450985724 |
                                          #                                                                                   state_d_934298480==363147933 | 
                                          #                                                                                   state_d_934298480==636706443 |state_d_934298480==771230670))) ))
...
alcl= dt_alclA  %>%  mutate(oth_age= case_when(D_633553324_D_294629316==265550580 ~"Didn't Drink", 
                                              ...
                                              TRUE ~ "Skipped this question"),
                            oth_age = factor(oth_age,levels=c("Didn't Drink","A few times per Year","A few times per Month",
                                                              "Once per Week", "A few times per Week","Once per day",
                                                              "2-3 times per Day","4-5 times per Day","6+ times per Day",
                                                              "Skipped this question")))
create_kable_table(alcl, oth_age, "ALCLIFE2A: Frequency of Drinking: Ages 0-17")


#ALCLIFE2B


dt_alclB= data_tib_all3  %>%   filter(D_429228540==132232896 & !is.na(D_556837046_D_141874857) & 
                                       (D_338467033==353358909 | (D_338467033==104430631 & !is.na(D_480426504))) & 
                                       ((is.na(D_480426504) & as.numeric(D_556837046_D_141874857)<24) | 
                                          (!is.na(D_480426504) & as.numeric(D_556837046_D_141874857)<24 & as.numeric(D_480426504)>24)))
                                          # (as.numeric(D_480426504)>17 | (is.na(D_48042650) & (state_d_934298480==124276120 |state_d_934298480==450985724 |
                                          #                                                                                   state_d_934298480==363147933 | 
                                          #                                                                                   state_d_934298480==636706443 |state_d_934298480==771230670))) ))
...
create_kable_table(alcl, oth_age, "ALCLIFE2B: Frequency of Drinking: Ages 18-24")



#ALCLIFE2C

dt_alclC= data_tib_all3  %>%   filter(D_429228540==132232896 & !is.na(D_556837046_D_141874857) & 
                                       (D_338467033==353358909 | (D_338467033==104430631 & !is.na(D_480426504))) & 
                                       ((is.na(D_480426504) & as.numeric(D_556837046_D_141874857)<29) | 
                                          (!is.na(D_480426504) & as.numeric(D_556837046_D_141874857)<29 & as.numeric(D_480426504)>29)))
                                          # (as.numeric(D_480426504)>17 | (is.na(D_48042650) & (state_d_934298480==124276120 |state_d_934298480==450985724 |
                                          #                                                                                   state_d_934298480==363147933 | 
                                          #                                                                                   state_d_934298480==636706443 |state_d_934298480==771230670))) ))
...
create_kable_table(alcl, oth_age, "ALCLIFE2C: Frequency of Drinking: Ages 25-29")


#ALCLIFE2D

dt_alclD= data_tib_all3  %>%   filter(D_429228540==132232896 & !is.na(D_556837046_D_141874857) &
                                       (D_338467033==353358909 | (D_338467033==104430631 & !is.na(D_480426504))) &
                                       ((is.na(D_480426504) & as.numeric(D_556837046_D_141874857)<39) |
                                          (!is.na(D_480426504) & as.numeric(D_556837046_D_141874857)<39 & as.numeric(D_480426504)>39)))
                                          # (as.numeric(D_480426504)>17 | (is.na(D_48042650) & (state_d_934298480==124276120 |state_d_934298480==450985724 |
                                          #                                                                                   state_d_934298480==363147933 |
                                          #                                                                                   state_d_934298480==636706443 |state_d_934298480==771230670))) ))
...
create_kable_table(alcl, oth_age, "ALCLIFE2D: Frequency of Drinking: Ages 30-39")
...
dt_alclE= data_tib_all3  %>%   filter(AGEGRID>40 & D_429228540==132232896 & !is.na(D_556837046_D_141874857) &
                                       (D_338467033==353358909 | (D_338467033==104430631 & !is.na(D_480426504))) &
                                       ((is.na(D_480426504) & as.numeric(D_556837046_D_141874857)<49) |
                                          (!is.na(D_480426504) & as.numeric(D_556837046_D_141874857)<49 & as.numeric(D_480426504)>49)))
                                          # (as.numeric(D_480426504)>17 | (is.na(D_48042650) & (state_d_934298480==124276120 |state_d_934298480==450985724 |
                                          #                                                                                   state_d_934298480==363147933 |
                                          #                                                                                   state_d_934298480==636706443 |state_d_934298480==771230670))) ))
...
create_kable_table(alcl, oth_age, "ALCLIFE2E: Frequency of Drinking: Ages 40-49")


#ALCLIFE2F

dt_alclF= data_tib_all3  %>%   filter(AGEGRID>50 & D_429228540==132232896 & !is.na(D_556837046_D_141874857) &
                                       (D_338467033==353358909 | (D_338467033==104430631 & !is.na(D_480426504))) &
                                       ((is.na(D_480426504) & as.numeric(D_556837046_D_141874857)<59) |
                                          (!is.na(D_480426504) & as.numeric(D_556837046_D_141874857)<59 & as.numeric(D_480426504)>59)))
                                          # (as.numeric(D_480426504)>17 | (is.na(D_48042650) & (state_d_934298480==124276120 |state_d_934298480==450985724 |
                                          #                                                                                   state_d_934298480==363147933 |
                                          #                                                                                   state_d_934298480==636706443 |state_d_934298480==771230670))) ))
...
create_kable_table(alcl, oth_age, "ALCLIFE2F: Frequency of Drinking: Ages 50-59")


#ALCLIFE2G

dt_alclG= data_tib_all3  %>%   filter(AGEGRID>60 & D_429228540==132232896 & !is.na(D_556837046_D_141874857) &
                                       (D_338467033==353358909 | (D_338467033==104430631 & !is.na(D_480426504))) &
                                       ((is.na(D_480426504) & as.numeric(D_556837046_D_141874857)<69) |
                                          (!is.na(D_480426504) & as.numeric(D_556837046_D_141874857)<69 & as.numeric(D_480426504)>69)))
                                          # (as.numeric(D_480426504)>17 | (is.na(D_48042650) & (state_d_934298480==124276120 |state_d_934298480==450985724 |
                                          #                                                                                   state_d_934298480==363147933 |
                                          #                                                                                   state_d_934298480==636706443 |state_d_934298480==771230670))) ))
...
create_kable_table(alcl, oth_age, "ALCLIFE2G: Frequency of Drinking: Ages 60-69")


#ALCLIFE2H-- no one yet

dt_alclH= data_tib_all3  %>%   filter(AGEGRID>70 & D_429228540==132232896 & !is.na(D_556837046_D_141874857) &
                                       (D_338467033==353358909 | (D_338467033==104430631 & !is.na(D_480426504))) &

                                       ((is.na(D_480426504) & as.numeric(D_556837046_D_141874857)<79) |
                                          (!is.na(D_480426504) & as.numeric(D_556837046_D_141874857)<79 &
                                             as.numeric(D_480426504)>79)))
                                          # (as.numeric(D_480426504)>17 | (is.na(D_48042650) & (state_d_934298480==124276120 |state_d_934298480==450985724 |
                                          #                                                                                   state_d_934298480==363147933 |
                                          #                                                                                   state_d_934298480==636706443 |state_d_934298480==771230670))) ))

alcl= dt_alclH  %>%  mutate(oth_age= case_when(D_633553324_D_602102163==265550580 ~"Didn't Drink",
                                                D_633553324_D_602102163==950039557 ~"A few times per Year",
                                                D_633553324_D_602102163==402048066 ~"A few times per Month",
                                                D_633553324_D_602102163==522395860 ~"Once per Week",
                                                D_633553324_D_602102163==756073829 ~"A few times per Week",
                                                D_633553324_D_602102163==858624942 ~"Once per day",
                                                D_633553324_D_602102163==424768954 ~"2-3 times per Day",
                                                D_633553324_D_602102163==687445652 ~"4-5 times per Day",
                                                D_633553324_D_602102163==333682419 ~"6+ times per Day",
                                              TRUE ~ "Skipped this question"),
                            oth_age = factor(oth_age,levels=c("Didn't Drink","A few times per Year",
                                                              "A few times per Month",
                                                              "Once per Week", "A few times per Week",
                                                              "Once per day",
                                                              "2-3 times per Day","4-5 times per Day",
                                                              "6+ times per Day",
                                                              "Skipped this question")))

create_kable_table(alcl, oth_age, "ALCLIFE2H: Frequency of Drinking: Ages 70+")


```


```{r Alclife2_Function, warning=FALSE, echo=FALSE, message=FALSE}


#------ NEW CODE I'M LOOKING TO UTILIZE ONCE FUNCTION FINALIZED ---------------------

alclife2_funct(0,  "ALCLIFE2A: Frequency of Drinking: Ages 0-17")
alclife2_funct(18, "ALCLIFE2B: Frequency of Drinking: Ages 18-24")
alclife2_funct(25, "ALCLIFE2C: Frequency of Drinking: Ages 25-29")
alclife2_funct(30, "ALCLIFE2D: Frequency of Drinking: Ages 30-39")
alclife2_funct(40, "ALCLIFE2E: Frequency of Drinking: Ages 40-49")
alclife2_funct(50, "ALCLIFE2F: Frequency of Drinking: Ages 50-59")
alclife2_funct(60, "ALCLIFE2G: Frequency of Drinking: Ages 60-69")
alclife2_funct(70, "ALCLIFE2H: Frequency of Drinking: Ages 70-79")
...
#ALCLIFE4B
alcl4B= dt_alclB  %>%  mutate(oth_age= case_when(D_470862706_D_643512687_D_789689151=="1"~"Drank Beer",
                                              ...
                                              TRUE ~ "Skipped this question"),
                             oth_age = factor(oth_age,levels=c("Drank Beer","Drank Liquor","Drank Wine","Drank Other Beverages", "Skipped this question")))

create_kable_table(alcl4B, oth_age, "ALCLIFE4B: Beverages Drank from Ages 18-24")
...
#ALCLIFE4C
alcl4C= dt_alclC  %>%  mutate(oth_age= case_when(D_470862706_D_688123102_D_789689151=="1"~"Drank Beer",
...
create_kable_table(alcl4C, oth_age, "ALCLIFE4C: Beverages Drank from Ages 25-29")
...
#ALCLIFE4D
alcl4D= dt_alclD  %>%  mutate(oth_age= case_when(D_470862706_D_356133766_D_789689151=="1"~"Drank Beer",
...
create_kable_table(alcl4D, oth_age, "ALCLIFE4D: Beverages Drank from Ages 30-39")
...
#ALCLIFE4E
alcl4E = dt_alclE  %>%  mutate(oth_age= case_when(D_470862706_D_690918725_D_789689151=="1"~"Drank Beer",
...
create_kable_table(alcl4E, oth_age, "ALCLIFE4E: Beverages Drank from Ages 40-49")
...
#ALCLIFE4F
alcl4F = dt_alclF  %>%  mutate(oth_age= case_when(D_470862706_D_178609656_D_789689151=="1"~"Drank Beer",
...
create_kable_table(alcl4F, oth_age, "ALCLIFE4F: Beverages Drank from Ages 50-59")
...
#ALCLIFE4G
alcl4G= dt_alclG  %>%  mutate(oth_age= case_when(D_470862706_D_261957180_D_789689151=="1"~"Drank Beer",
...
create_kable_table(alcl4G, oth_age, "ALCLIFE4G: Beverages Drank from Ages 60-69")

```

### ALCLIFE4H: Beverages Drank from Ages 70-79 
```{r Alclifeh, message=FALSE, echo=FALSE, warning=FALSE}
#ALCLIFE4G
alcl4H= dt_alclH  %>%  mutate(oth_age= case_when(D_470862706_D_715827646_D_789689151=="1"~"Drank Beer",
                                              D_470862706_D_715827646_D_663253668=="1"~"Drank Liquor",
                                              D_470862706_D_715827646_D_104676242=="1"~"Drank Wine",
                                              D_470862706_D_715827646_D_137733407=="1"~"Drank Other Beverages",
                                              TRUE ~ "Skipped this question"),
                             oth_age = factor(oth_age,levels=c("Drank Beer","Drank Liquor","Drank Wine","Drank Other Beverages", "Skipped this question")))

create_kable_table(alcl4H, oth_age, "ALCLIFE4H: Beverages Drank from Ages 70-79")

```


```{r Clear_Memory3, include=FALSE}
#Clearing up some memory -- removing all but the main dataframe and functions
rm(list = setdiff(ls(), c("data_tib_all3", 'module3', "dict", 'hist_funct', 'boxp_funct', 'simple_funct3', 'simple_age_funct3', 'one_logic_funct3', 'alclife2_funct', "alclife_bands", "alclife_levels", 'funct_2_3_4', 'funct_5', 'funct_8c', 'funct_9b', 'funct_10_11', 'create_kable_table', 'create_kable_table_w_footnote', "render_kable", "build_summary")))
invisible(gc())
...
sscreen2 <- sscreen %>%  mutate(answer= case_when(D_711858159==905073380~"All sun exposed areas",
                                              D_711858159==223461539~"Some sun exposed areas",
                                              D_711858159==901521065~"Face/Head only",
                                              TRUE ~ "Skipped this question"),
                                answer = factor(answer,levels=c("All sun exposed areas","Some sun exposed areas","Face/Head only", "Skipped this question")))

create_kable_table(sscreen2, answer, "SUNCREEN2: Body Parts Sunscreen Applied To")



#SUNCREEN3
sscreen3 <- sscreen %>%  mutate(answer= case_when(D_467751479==648960871~"Never",
                                              D_467751479==235613392~"Rarely",
                                              D_467751479==132099255~"Sometimes",
                                               D_467751479==211590917~"Often",
                                              D_467751479==907579123~"Always",
                                              TRUE ~ "Skipped this question"),
                                answer = factor(answer,levels=c("Never","Rarely","Sometimes", "Often","Always","Skipped this question")))

create_kable_table(sscreen3, answer, "SUNCREEN3: Frequency of Re-applying Sunscreen When Out in the Sun")



#SUNCREEN4
sscreen4 <- sscreen %>%  mutate(answer= case_when(D_858858333==445549128~"Less than 15",
                                              ...
                                              TRUE ~ "Skipped this question"),
                                answer = factor(answer,levels=c("Less than 15","15 to 20","21 to 35", "36 to 50","Higher than 50","Unknown","Skipped this question")))

create_kable_table(sscreen4, answer, "SUNCREEN3: Frequency of Re-applying Sunscreen When Out in the Sun")
...
#TANBED3
dttanbed3 <- data_tib_all3 %>%  
  filter(D_662762705==649327523| D_662762705==587052987 | D_662762705==838336013) %>%  
  mutate(answer= case_when(D_605063358==104430631 ~ "No",
                           D_605063358==353358909 ~ "Yes",
                           is.na(D_605063358) ~ "Skipped this question"))

create_kable_table(dttanbed3, answer, "TANBED3: Used a Tanning Bed in the Past 12 Months")
...
#TANBED5
dttanbed5 <- data_tib_all3 %>%  
  filter(D_662762705==649327523| D_662762705==587052987 | D_662762705==838336013) %>% 
  mutate(answer= case_when(D_466473224==676516297 ~ "20 to 31 days per month",
         ...
         answer = factor(answer,levels=c("20 to 31 days per month","5 to 19 days per month","Once per week", 
                                         "1 to 4 days per month","0 to 11 days per year","Skipped this question")))

create_kable_table(dttanbed5, answer, "TANBED5: Used a Tanning Bed in the Past 12 Months")

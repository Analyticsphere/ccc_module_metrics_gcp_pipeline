---
title: "PROMIS Backlog Duplicates"
...
Dup_All_Sites <- read.csv("Dup_Crosswalk_All_Sites.csv")
#https://nih.app.box.com/file/1648745247869
...
d_949302066, d_517311251, d_536735468, d_832139544, d_976570371, d_770257102, d_663265240, d_264644252, d_320303124, d_843688458
...
dups_qol= as_tibble(data_c)


colnames(dups_qol) <- c("token", "Connect_ID", "Site", "verif_date", "verif_status", "dup_status", "sigin_time", "method",
...
## Need a common participant identifier for each Connect_ID with an original and duplicate account. PT_label was created in the excel file for just that


dups_qol$Connect_ID <- as.numeric(dups_qol$Connect_ID) 
...
dups_qol_combined <- dups_qol %>%
 ...
 # Second join: match dups_qol$Connect_ID with Dup_All_Sites$Duplicate_Connect_ID
 dups_qol_combined <- dups_qol_combined %>%
   left_join(Dup_All_Sites %>%
...
# View the result
#head(dups_qol_combined)


dups_qol_combined <- dups_qol_combined %>%  mutate(dup_status = case_when(dup_status=="654558118" ~ "Duplicate Account",
                                 ...
                                 QOL_status= case_when(QOL_status=="972455046" ~ "Not Started",
                                                       QOL_status=="615768760" ~ "Started",
                                                       QOL_status=="231311385" ~ "Submitted",
                                                       is.na(QOL_status) ~ "Missing Flags"))
...
method_by_acct_type <- dups_qol_combined  %>%
  group_by(PT_label) %>%
...
method_comparison <- dups_qol_combined %>%
  group_by(PT_label) %>%
  mutate(
...
Last_Survey_table <- dups_qol_combined %>%
  group_by(PT_label) %>%
...
#Comparable table to previous, among those verified prior to 12/1/23, with PROMIS Completed and not in the duplicate list 

sql_no_dup <- paste0("SELECT  token,Connect_ID, d_827220437, d_914594314, d_821247024, state_d_148197146, d_335767902, d_995036844,
d_949302066, d_517311251, d_536735468, d_832139544, d_976570371, d_770257102, d_663265240, d_264644252, d_320303124, d_843688458
FROM `nih-nci-dceg-connect-prod-6d04.FlatConnect.participants` where DATE(d_914594314)<'2023-12-01' and d_320303124='231311385' and 
...
Knit_No_Dup <- knitr::kable(Last_Survey_No_Dup, 
                            ...
                            row.names = FALSE, align = c("l", "c", "c", "c", "c", "c", "c"),
                            booktabs = TRUE) 
add_footnote(Knit_No_Dup, "This table includes participants verified prior to 12/1/23, with PROMIS completed, and without duplicates.", notation = "none")

# Knit_No_Dup_with_footnote <- add_footnote(Knit_No_Dup,
#                                           "\\small {This table includes participants verified prior to 12/1/23, with PROMIS completed, and without duplicates.}",
#                                           notation = "none")
# 
...
```{r QOL_Comp, echo=FALSE, warning=FALSE, message=FALSE, results='asis'}


QOL_OG <- dups_qol_combined %>%
  filter(dup_status == "Original Account") %>% 
  mutate(QOL_comp = QOL_status)



QOL_OG_summary <- QOL_OG %>%
  group_by(QOL_status) %>%
  summarize(
    Frequency = n(),
    Percentage = round((Frequency / nrow(QOL_OG)) * 100, digits=2)
  ) %>%
  arrange(desc(Frequency)) %>% 
  ungroup %>% 
  bind_rows(
...
colnames(QOL_OG_summary)[1] <- ""

knitr::kable(QOL_OG_summary, 
             caption='Table 3. PROMIS Survey Completion Status on Original Account', 
             row.names=FALSE,align=c("l","c","c"),
             booktabs = TRUE) %>%  
  add_indent(seq(1, nrow(QOL_OG_summary) - 1))  %>% 
  kable_styling(latex_options = "scale_down")

...
connect_ids <- data.frame(Connect_ID = c('8176277977'))
...
token <- "4bcdd313-d0bd-4391-bb76-eb95d17df92f"
...
write.csv(stub_df, "query1_table1_stub_counts.csv", row.names = FALSE)
write.csv(nonstub_df, "query1_table1_nonstub_counts.csv", row.names = FALSE)
...
for (table in tables) {
  ...
  sql_query <- sprintf(
    "SELECT *
     FROM `nih-nci-dceg-connect-stg-5519.FlatConnect.%s`
     WHERE Connect_ID IN (%s)",
...
count_nulls_non_nulls_tokens <- function(table, tokens) {
  ...
  sql_query <- sprintf("SELECT * FROM `nih-nci-dceg-connect-stg-5519.FlatConnect.notifications_JP` WHERE token IN (%s)", paste(sprintf("'%s'", token), collapse = ", "))
...
write.csv(all_counts, "query2_all_table_counts.csv", row.names = FALSE)
...
sql_query <- sprintf(
  "SELECT d_820476880, Connect_ID
   FROM `nih-nci-dceg-connect-stg-5519.FlatConnect.biospecimen_JP`
   WHERE Connect_ID IN (%s)",
...
sql_query <- "SELECT * FROM `nih-nci-dceg-connect-stg-5519.FlatConnect.boxes_JP`"
...
write.csv(wb2, "query3_ssn_boxes", row.names = FALSE)


## If they didn't answer the Boxes table, but did do SSN table
addWorksheet(wb2, "ssn_counts_df")
writeData(wb2, "ssn_counts_df", ssn_counts_df)

# Save the workbook to a file
#saveWorkbook(wb2, filename, overwrite = TRUE)
write.csv(wb2, "ssn_counts_df", row.names = FALSE)

...
parts <- "SELECT Connect_ID, d_564964481, d_795827569, d_544150384, d_117249500, d_454445267,d_949302066 FROM `nih-nci-dceg-connect-prod-6d04.FlatConnect.participants` 
...
wd_ver <- "SELECT Connect_ID,d_821247024,d_747006172  FROM `nih-nci-dceg-connect-prod-6d04.FlatConnect.participants` 
...
The question RACEETH was used for race and ethnicity. It is an optional question on Module 1; those who did not answer were classified as Unknown.

The question on SEX was used for this report. It is an optional question on Module 1; those who did not answer were classified as Unknown.
...
table2csv <- irb %>%
  group_by(race, race_hisp, sex_at_birth, age) %>%
  arrange(race, race_hisp, sex_at_birth, age) %>% 
  select(race, race_hisp, sex_at_birth, age) %>% 
  mutate(age_unit = "Years")

colnames(table2csv) <- c("Race", "Ethnicity", "Sex", "Age", "Age Unit")

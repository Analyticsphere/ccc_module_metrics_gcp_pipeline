...
d_906417725,d_773707518,d_831041022,d_987563196, d_659990606, 
...
if (nrow(reinvite_date_type) == 0) {
  ...
  reinvite_date_type <- reinvite_date_type  %>% select(Site, Connect_ID, d_439351436, d_280021666)  %>%  safe_arrange(Site)
...
if (nrow(start_DHQ) == 0) {
  ...
  start_DHQ <- start_DHQ %>% select(Site, Connect_ID, d_109610692, d_176068627) %>%  safe_arrange(Site)
...
if (nrow(comp_DHQ) == 0) {
  ...
  comp_DHQ <- comp_DHQ %>% select(Site, Connect_ID, d_610227793, d_176068627) %>%  safe_arrange(Site)
...
```{r elg_NE_UC, echo=FALSE, warning=FALSE, message=FALSE}

# \FloatBarrier
# ## 59. If payment issued = yes and site NE UC, then NORC payment eligible = yes
# 
# elg_NE_UC <- partsbq %>%  filter(d_130371375_d_266600170_d_648936790=='353358909'	 & d_827220437=='809703864' & 
#                                                d_130371375_d_266600170_d_222373868=='104430631')
# 
# 
# if (nrow(elg_NE_UC) == 0) {
#   cat("No errors\n")
# } else {
#   
#   elg_NE_UC <- elg_NE_UC %>% select(Site, Connect_ID, d_130371375_d_266600170_d_648936790, d_130371375_d_266600170_d_222373868) %>%  safe_arrange(Site)
# 
#   colnames(elg_NE_UC) <- c("Site", "Connect ID", "HDPaym_PaymIssuedBL", "SMPaym_NORCPaymEligBL")
# 
#   knitr::kable(elg_NE_UC) %>%
#     kable_styling(latex_options = "scale_down")
# }

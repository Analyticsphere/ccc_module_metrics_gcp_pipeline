...
# act_di_SF_race <- act_di_SF_race %>% select(Site, Connect_ID, token, d_747006172, d_821247024, state_d_793822265, state_d_119643471, state_d_934298480, state_d_706256705)
# colnames(act_di_SF_race) <- c("Site", "Connect ID", 'token', 'HdWd_WdConsent_v1r0', "RcrtV_Verification_v1r0", "RcrtV_UpdateRecType_v1r0", "RcrtSI_SHRace_v1r0", "RcrtSI_Age_v1r0", "RcrtSI_Sex_v1r0")
# 
# act_di_SF_race <- act_di_SF_race %>% mutate(RcrtV_Verification_v1r0 = case_when(RcrtV_Verification_v1r0=='875007964'	~ "Not yet verified",
...
if (nrow(act_di_SF_race) == 0) {
  ...
  act_di_SF_race <- act_di_SF_race %>% select(Site, Connect_ID, d_821247024, state_d_793822265) %>%  safe_arrange(Site)

  colnames(act_di_SF_race) <- c("Site", "Connect ID", 'RcrtV_Verification', "RcrtV_UpdateRecType")
...
if (nrow(act_di_member) == 0) {
  ...
  act_di_member <- act_di_member %>% select(Site, Connect_ID, d_821247024, state_d_793822265) %>%  safe_arrange(Site)

  colnames(act_di_member) <- c("Site", "Connect ID", 'RcrtV_Verification', "RcrtV_UpdateRecType")
...
if (nrow(act_di_elg) == 0) {
  ...
  act_di_elg <- act_di_elg %>% select(Site, Connect_ID, d_821247024, state_d_793822265) %>%  safe_arrange(Site)

  colnames(act_di_elg) <- c("Site", "Connect ID", 'RcrtV_Verification', "RcrtV_UpdateRecType")
...
## 87. UTM_Campaign parameter for each recruit should match the data we receive for A/B Testing Message Response.
...
#                                                             state_d_956485028== '477331464'	~ 'Cancer Connection Personal',
#                                                             state_d_956485028== '935486262'	~ 'Cancer Connection General',
#                                                             state_d_956485028== '518814501'	~ 'Research Personal',
#                                                             state_d_956485028== '307763550'	~ 'Research General'),
#                                     UTM_camp = gsub("-", " ", d_163847117)) %>%
#   filter(tolower(AB_response) != tolower(UTM_camp))
# 
# 
# if (nrow(AB1) == 0) {
#   cat("No errors\n")
...
## 88. If any one or two of the three UTM parameters are populated, all three should be available. 

# AB2 <- partsbq	%>% filter((is.na(d_761057722) | is.na(d_207613315) | is.na(d_163847117)) & #at least one is missing

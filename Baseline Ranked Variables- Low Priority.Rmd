...
# orient= sexorient  %>%  mutate(sexuality= case_when(length(grep("asexual", sexorient$D_555481393_D_979809707)) ~ "Asexual",
#                                                            length(grep("pansexual", sexorient$D_555481393_D_979809707))~ "Pansexual",
#                                                            length(grep("queer", sexorient$D_555481393_D_979809707)) ~ "Queer",
#                                                            length(grep("straight", sexorient$D_555481393_D_979809707)) ~ "Straight",
#                                                           is.na(D_555481393_D_979809707)~ "Skipped this Question",
#                                                       TRUE ~ D_555481393_D_979809707))

# sexorient= sexorient  %>%  mutate(sexuality= case_when(gsub(" ", "", sexorient$gsub(" ", "", sexorient$D_555481393_D_979809707))=="asexual" ~ "Asexual",
#                                                     gsub(" ", "", sexorient$D_555481393_D_979809707)=="pansexual" | gsub(" ", "", sexorient$D_555481393_D_979809707)=="pan" ~ "Pansexual",
#                                                     gsub(" ", "", sexorient$D_555481393_D_979809707)=="queer"~ "Queer",
#                                                     gsub(" ", "", sexorient$D_555481393_D_979809707)=="straight" | gsub(" ", "", sexorient$D_555481393_D_979809707)=="hetero"~ "Straight",
#                                                     is.na(gsub(" ", "", sexorient$D_555481393_D_979809707))~ "Skipped this Question",
#                                                       TRUE ~ D_555481393_D_979809707))
# 
# 
# sexorient= sexorient  %>%  mutate(sexuality= case_when(grep("asexual",  sexorient$sexuality_no_spaces, value=TRUE) ~ "Asexual",
#                                                            grep("pan",  sexorient$sexuality_no_spaces, value=TRUE)~ "Pansexual",
#                                                            grep("queer",  sexorient$sexuality_no_spaces, value=TRUE)~ "Queer",
#                                                            grep("straight",  sexorient$sexuality_no_spaces, value=TRUE) ~ "Straight",
#                                                           is.na(D_555481393_D_979809707)~ "Skipped this Question",
#                                                       TRUE ~ "Other"))
...
# sexorient= sexorient  %>%  mutate(sexuality= case_when(D_555481393_D_979809707=="asexual" | D_555481393_D_979809707=="a sexual" ~ "Asexual",
#                                                            D_555481393_D_979809707=="pansexual" | D_555481393_D_979809707=="pan sexual" | D_555481393_D_979809707=="pan"~ "Pansexual",
#                                                            D_555481393_D_979809707=="queer"~ "Queer",
#                                                            D_555481393_D_979809707=="straight" | D_555481393_D_979809707=="hetero" ~ "Straight",
#                                                           is.na(D_555481393_D_979809707)~ "Skipped this Question",
#                                                       TRUE ~ "Other"))
...
#gsub(" ", "", x) <- remove spaces so its easier to find any version of pansexual, ect. 
#length(grep("corn", dataset)) <- grab all versions of asexual or pansexual, and count them
 



# sexorient %>%  select(D_555481393_D_979809707) %>% filter(!is.na(D_555481393_D_979809707)) %>% gt() %>% cols_label(D_555481393_D_979809707= md("**Response for 'Other' Sexual Orientation**"))

#sexorient_table <- sexorient %>%  select(age, race, sexuality) %>%  group_by(age, race) %>% filter(!is.na(sexuality) & age!="Skipped this Question" & race!="Skipped this Question") %>% ungroup() %>% gt() %>% cols_label(age="Age", race="Race", sexuality= md("**Response for 'Other' Sexual Orientation**")) 
...
sexorient_table2 <- sexorient %>% filter(!is.na(D_555481393_D_979809707) & age!="Skipped this Question") %>% dplyr::group_by(age,  sexuality) %>% dplyr::summarize(n=n(), percentage=100*n/nrow(.)) %>% dplyr::ungroup()  %>%  dplyr::select(age, sexuality, n, percentage)

sexorient_age_gt <- sexorient_table2 %>%  gt::gt(rowname_col = "row_lab") %>%  
  cols_label(n= md("**N**"), age="Age", sexuality="'Other Sexuality' Response", percentage = md("**%**")) %>% 
  grand_summary_rows(columns=c(n, percentage),fns = ~sum(.,na.rm = T))|>
  tab_options(
      stub.font.weight = "bold"
    ) 
...
sexorient_table <- sexorient %>% filter(!is.na(D_555481393_D_979809707) & age!="Skipped this Question") %>% dplyr::group_by(sexuality) %>% dplyr::summarize(n=n(), percentage=100*n/nrow(.)) %>% dplyr::ungroup()  %>%  dplyr::select(sexuality, n, percentage)

sexorient_table %>%  gt::gt(rowname_col = "row_lab") %>%  
  fmt_number(columns = "percentage", decimals = 2) %>%
  cols_label(n= md("**N**"), sexuality=md("**'Other Sexuality' Response**"), percentage = md("**%**")) %>% 
  grand_summary_rows(columns=c(n, percentage),fns = ~sum(.,na.rm = T))|>
  tab_options(
      stub.font.weight = "bold"
    ) 
...
```{r altadd2, warning=FALSE, echo=FALSE, message=FALSE}
...
# ALTCONTACT1,2
```{r altcontact12, warning=FALSE, echo=FALSE, message=FALSE}

#ALTADDRESS1/ALTADDRESS2/ALTCONTACT1/ALTCONTACT2 won't list for PII reasons

  altcont1= data_tib_base %>%  mutate(Contact= case_when(!is.na(D_986275155_D_661719912) & !is.na(D_986275155_D_801653230)~"Gave Full Name",
...
altcont2= data_tib_base %>%  mutate(Contact= case_when(!is.na(D_285718391_D_286149234) | !is.na(D_285718391_D_318130543) | !is.na(D_285718391_D_750097000)~"Gave Alternative Contact Info",

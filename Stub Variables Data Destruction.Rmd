---
title: "Stub Variables Data Destruction"
...
#change as necessary to check prod-6d04 or stg-5519
...
DDoutputpath <-"~/Documents/GitHub/ccc_module_metrics_gcp_pipeline/"
...
# INNER JOIN `",project,".FlatConnect.participants_JP` p ON cast(b.Connect_ID as int64)= cast(p.Connect_ID as int64)
...
##Need this part to identify all the stub variables with labels
...
for (t in c(1:length(flatdatasets))){
    ...
    query <-  eval(parse(text=paste("bq_project_query(project, query=\"SELECT * FROM `",project,".FlatConnect`.
...
### JING, THIS PART DOESN'T RUN FOR ME: 
  ## ERROR: Table nih-nci-dceg-connect-dev:FlatConnect.QC_report was not found
...
##This creates labels- likely don't need for testing 
...
#JING- this doesn't run for me. May need to update the outpath. Is this part necessary?
...
##Selects stub records- needed for last chunk
...
##This is checking individual tables with individual connect IDs. Would need to make a list of CIDs to go through everyone

##Can do this for all three scenarios. Save number of rows before destruction and then compare afterward

##Example for multiple people, also seen below
...
# check1_stud_dd <- merge(check1_stubs, dd.y[,c("conceptId.2","conceptId.3", "Variable.Label","Variable.Name","Current.Source.Question","Current.Question.Text")], by.x="description",by.y="conceptId.3",all.x=TRUE) %>% as_tibble() %>%
#     mutate(select = ifelse(is.na(conceptId.2),1,ifelse(str_count(field_path,as.character(conceptId.2)) > 0, 1,0))) %>% filter(.,select==1) %>% group_by(field_path) %>% arrange(field_path) %>% distinct(field_path,.keep_all = TRUE)

#print(check1_stud_dd[,colSums(is.na(check1_stud_dd)) == 0])

#submitted 4-digit SSN
ids2 <- 7204238865 # check the SSN variable
...
##Repeated from above, functions pulled out
...
#here is the data before DD  before Aileen assigned the data destruction form
  ...
  ##All .t tables are the longated ConnectID by ConnectID row
...
#Ad-hoc, go table by table with all varaibles to check participants if needed 
...
##Might be able to change thi chunk for rule 1, would start at fatdatasets
...
dd_tb_list <- dd_tb_list %>% 
  mutate(dd_qualified = ifelse(d_883668444=='704529432' | days_DDrequests<60, 1,0))  #days_DDrequests>=60, 1,0))
...
##looks at all data tables
flatdatasets <- dbListTables(con)

## update this based on #2 or #3
ls_tobe_dd <- dd_tb_list$Connect_ID[which(dd_tb_list$dd_qualified==1 & dd_tb_list$d_861639549=='104430631')] 

## stub records destroyed or not
new.dd.ls <- list()
for (n in ls_tobe_dd){
  id <- n
  dt_destr_tobe <- NULL
    for (t in c(2,3,5:18, 20)){
        x <- flatdatasets[t]
        y <- "FlatConnect"
        id <- n
        #print(x)

---
title: "U-Chicago Module 1 Summary Statistics"
...
# ##Recruitment Table variables added for merge:
##

# D_521025370 #Time Stamp: study ID submitted site
# D_512820379 #Recruitment type flag
# D_471593703 #Timestamp: Recruitment flag
# D_934298480 #Site reported age at recruitment
# D_230663853 #Sign-in flag
# D_335767902 #Timestamp: sign-in flag
# D_982402227 #Date consent form submitted
# D_919254129 #Consent submitted flag
# D_699625233 #User profile submitted flag
# D_564964481 #Birth month
# D_795827569 #Birth day
# D_544150384 #Birth year
# D_371067537 #DOB derived from concatenation 
# D_430551721 #Timestamp: user profile submitted
# D_821247024 #Verification by site
# D_914594314 #Timestamp: verification update
# D_147176963 #Verification of f_D_name
# D_557461333 #verif of l_D_name
# D_725929722 #verif of dob
# D_711794630 #verif of pin
# D_679832994 #verif of token
# D_570452130 #eligibility variable site
# D_629484663 #eligibility age
# D_547895941 #eligibility cancer status
# D_949302066 #Mod 1 flag
# D_517311251 #Timestamp: Mod1 completion
# D_205553981 #Timestamp: Mod1 start 
...
module1$lete <- m1_combined_v1v2[which(m1_combined_v1v2$Connect_ID %in% recr_m1$Connect_ID[which(recr_m1$d_949302066 ==231311385 )]),] #7289 including duplicates 05022023

module1$lete <- module1$lete %>% arrange(desc(version)) 


module1$lete_nodup <- module1$lete[!duplicated(module1$lete$Connect_ID),] 
table(module1$lete_nodup$version)
...
D_949302066 , D_517311251, D_205553981, D_117249500, d_430551721, d_517311251, d_544150384, d_564964481, d_117249500 FROM `nih-nci-dceg-connect-prod-6d04.FlatConnect.participants` 
...
module1= left_join(module1$lete_nodup, parts_data, by="Connect_ID") 
dim(module1)
...
module1 <- module1 %>%  filter(d_827220437=='809703864')
...
##Age 
#Derived Age
#table(module1$D_117249500) #only going to be applied to participants finishing module 1 after derived variable implemented 


##Age Verification
simple_funct(D_479353866,"AGECOR: Age Reported Correct")

module1$DOB <- paste0(str_sub(module1$D_371067537, 1,4), "-", str_sub(module1$D_371067537, 5,6), "-", str_sub(module1$D_371067537, 7,8))
module1$calculated <- round(time_length(difftime(module1$D_205553981, module1$DOB), "years"), digits=3)
difference_age <- module1 %>%  filter(!is.na(D_150344905) & str_sub(calculated,1,2)!=D_150344905) %>% select(calculated, D_150344905)
...
# ## Checking the reason for so many age corrections
# 
# module1$d_517311251 <- as.Date(module1$d_517311251)
# module1$d_430551721 <- as.Date(module1$d_430551721)
# module1$bday <- round(time_length(difftime(module1$d_517311251, module1$d_430551721), "months"), digits=1)
# 
# 
# 
# bthday <- module1 %>% filter(!is.na(D_150344905)) #%>% select(bday) #$bday[!is.na(module1$D_479353866)]
# 
# summary(bthday)
# 
# 
# module1 %>% mutate(bday:=as.numeric(bday)) %>%
#      filter(!is.na(module1$D_150344905)) %>%
#      ggplot(aes(x=bday))+
#      xlab("Months") + geom_boxplot() +  ggtitle("Months From User Profile Completion to Module 1 Completion")
# 
# table(bthday>3)
# 
# 
# table(module1$D_150344905[module1$D_479353866==353358909])
# table(module1$d_517311251[!is.na(module1$D_150344905)])
# table(is.na(module1$d_544150384)[!is.na(module1$D_150344905)])
# table(is.na(module1$d_564964481)[!is.na(module1$D_150344905)])
# 
# table(as.numeric(module1$d_117249500, na.rm=T)!=as.numeric(module1$D_150344905, na.rm=T))
# 
# derived <- module1 %>% filter(!is.na(module1$d_117249500) & !is.na(module1$D_150344905))
# 
# table(as.numeric(derived$d_117249500)!=as.numeric(derived$D_150344905))
# table(module1$Connect_ID[as.numeric(derived$d_117249500)!=as.numeric(derived$D_150344905)])
# 
#module1$DOB <- paste0(str_sub(module1$D_371067537, 1,4), "-", str_sub(module1$D_371067537, 5,6), "-", str_sub(module1$D_371067537, 7,8))
# 
# age_conf <- module1 %>%  filter(!is.na(D_150344905)) %>%  select(d_430551721, DOB, D_205553981)
# View(age_conf)
# 
# 
#module1$calculated <- round(time_length(difftime(module1$D_205553981, module1$DOB), "years"), digits=3)
# #module1$given <- round(time_length(difftime(module1$d_517311251, module1$d_430551721), "months"), digits=1)
# 
# age_compare <- module1 %>%  filter(!is.na(D_150344905)) %>% select(calculated, D_150344905)
# 
# View(age_compare)
# 
# 
# difference_age <- module1 %>%  filter(!is.na(D_150344905) & str_sub(calculated,1,2)!=D_150344905) %>% select(calculated, D_150344905)
# cat("Entered an Age Different Than What was Calcu:", dim(difference_age)[[1]])
# colnames(difference_age) <- c("Age Calculated", "Age Inputted at AGE")
# View(difference_age)
...
which_lang= data_tib_m1 %>%  mutate(language= case_when(D_588212264_D_588212264_D_163149180==1 ~ "English",
...
#Appearance/Style
style= data_tib_m1  %>%  mutate(descr= case_when(D_987107433==220083334 ~ "Very Feminine",
...
dt_skincnc_type_summary %>%  gt::gt(rowname_col = "row_lab") %>%  
  ...
  cols_label(n= md("**Number of Participants**"),Type = md("**Type**"), percentage = md("**Percentage of Participants**")) %>% 
  # summary_rows(
  #   column = c(participants,percentage),
  #   fns= list(
  #     Sum= ~sum(.)
...
dt_breast_sg_summary <- mhgroup9 %>% group_by(breast_sg) %>%  dplyr::summarize(n=n(), percentage=100*n/nrow(.))  %>% dplyr::ungroup() %>%  dplyr::select(breast_sg, n, percentage)

dt_breast_sg_summary %>%  gt::gt(rowname_col = "row_lab") %>%  
  fmt_number(columns = "percentage", decimals = 2) %>% 
  tab_header(title = md("BREASTSUR: Types of Breast Surgeries Had")) %>% 
  cols_label(n= md("**Number of Participants**"), breast_sg = md("**Type**"), percentage = md("**Percentage of Participants**")) %>% 
  # summary_rows( groups=F,
  #   column = c(n,percentage),
  #   fns= list(
  #     Sum= ~sum(.)
...
tmpM2 <- tmpMC2[, c(8,2,3,12,4,1,9, 5, 7, 10, 6)]
Bsurgnc_dx <- NULL
for (i in 1:11){
...
health= data_tib_m1   %>%  mutate(status= case_when(D_875208305==131550264 ~ "Fair",
...
dt_overall_summary %>%  gt::gt(rowname_col = "row_lab") %>%  
  fmt_number(columns = "percentage", decimals = 2) %>% 
...
#Scale
dt_painsc= data_tib_m1   %>%  filter(D_323512813==353358909)
...
dt_painsc_summary %>%  gt::gt(rowname_col = "row_lab") %>%  
  fmt_number(columns = "percentage", decimals = 2) %>% 
  tab_header(title = md("PAIN2: Rank of Pain")) %>% 
  cols_label(n= md("**Number of Participants**"), descr = md("**Rating**"), percentage = md("**Percentage of Participants**")) %>% 
  # summary_rows( groups=F,
  # column = c(n,percentage),
  # fns= list(
  #   Sum= ~sum(.)
...
#Affected Work?
dt_painaff= data_tib_m1   %>%  filter(D_323512813==353358909)
...
dt_painaff_summary %>%  gt::gt(rowname_col = "row_lab") %>%  
  fmt_number(columns = "percentage", decimals = 2) %>% 
  tab_header(title = md("PAIN3: Pain Affected Work")) %>% 
  cols_label(n= md("**Number of Participants**"), status = md("**Answer**"), percentage = md("**Percentage of Participants**")) %>% 
  # summary_rows( groups=F,column = c(n,percentage), 
  #              fns= list( Sum= ~sum(.)), decimals=0) %>%
  grand_summary_rows(columns=c(n, percentage),fns = ~sum(.,na.rm = T))|>
  tab_options(stub.font.weight = "bold") %>%
  tab_style(style = list(cell_text(weight = "bold")),
            locations = cells_body(columns = status,)) %>%
...
#height at 10
dict <- c(dict, "325478778"="Shorter than average","730757606"="About avg", "897599966"="Taller than average")
simple_funct(D_650399623, "AVEHIGHT: Height at Age 10")

#weight at 10
dict <- c(dict, "176904165"="Thinner than Average","902131456"="About Average Build", "839266527"="Heavier than Average")
...
dt_wt_carry_summary %>%  gt::gt(rowname_col = "row_lab") %>%  
  ...
  cols_label(n= md("**Number of Participants**"), weight = md("**Body Part(s)**"), percentage = md("**Percentage of Participants**")) %>% 
    # summary_rows( groups=F,
    # column = c(n,percentage),
    # fns= list(
    #   Sum= ~sum(.)
...
dt_fem_hair= data_tib_m1   %>% filter(D_407056417==536341288) 
...
dt_fem_hair_summary %>%  gt::gt(rowname_col = "row_lab") %>%  
  fmt_number(columns = "percentage", decimals = 2) %>% 
  tab_header(title = md("Level of Hair Loss (Female)")) %>% 
  cols_label(n= md("**Number of Participants**"), hair = md("**Hair Loss**"), percentage = md("**Percentage of Participants**")) %>% 
  # summary_rows( groups=F,
  # column = c(n,percentage),
  # fns= list(
  #   Sum= ~sum(.)
...
dt_male_hair= data_tib_m1   %>% filter(D_407056417==654207589) 
...
dt_male_hair_summary %>%  gt::gt(rowname_col = "row_lab") %>%  
  fmt_number(columns = "percentage", decimals = 2) %>% 
...
dt_sibcanc1_summary %>%  gt::gt(rowname_col = "row_lab") %>%  ## why is this not a proper gt_tbl??
  ...
  cols_label(status = md("**Diagnosed**"), n = md("**Number of Participants**"), percentage = md("**Percentage of Participants**")) %>%
    # summary_rows(
    # column = c(n,percentage),
    # fns= list(
    #   Sum= ~sum(.)
...
sibling_bd_canc <- dt_body_sibling_summary %>%  gt::gt(rowname_col = "row_lab") %>%  
  ...
  cols_label(bodyp = md("**Cancer**"), percentage = md("**Percentage of Participants**")) %>% 
...
```{r Section5, warning=FALSE, echo=FALSE, message=FALSE}
...
dt_curr_bus_summary %>%  gt::gt(rowname_col = "row_lab") %>%  
  ...
  cols_label(n= md("**Number of Participants**"), category = md("**Type**"), percentage = md("**Percentage of Participants**")) %>% 
  # summary_rows( groups=F,
  #   column = c(n,percentage),
  #   fns= list(
  #     Sum= ~sum(.)
...
module1 %>% mutate(D_739294356=as.numeric(as.character(D_739294356))) %>%
    ...
    geom_histogram(color="black")+ scale_y_continuous(breaks=pretty_breaks()) +
    geom_vline(aes(xintercept=mean(D_739294356, na.rm=T)), colour="red", linetype="solid")
...
dt_income_summary %>%  gt::gt(rowname_col = "row_lab") %>%  
  fmt_number(columns = "percentage", decimals = 2) %>% 
...
cat("Don't know number of people in household:", sum(table(module1$D_641651634_D_641651634_D_746038746==1)))
...
#module1 survey 
dict <- c(dict, "275706407"="Someone on behalf of Participant", "589702540"="Completed by Participant")

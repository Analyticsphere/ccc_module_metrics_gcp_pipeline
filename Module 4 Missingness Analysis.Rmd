...
# dt_SEASADDYRS_summary %>%  gt::gt() %>%  #gt::gt(rowname_col = "row_lab") %>%
#   fmt_number(columns = "percentage", decimals = 2) %>%
...
#gt::set_default_latex_engine("gt_latex_limited")

gt_tbl <- dt_SEASADDYRS_summary %>%
  gt() %>%
  fmt_number(columns = "percentage", decimals = 2) %>%
  tab_header(title = "Years and Months at Vacation Home 1") %>%
  cols_label(
  ...
  tab_style(
    ...
    locations = cells_body(columns = Address1)
  )

cat(as.character(gt::as_latex(gt_tbl)))

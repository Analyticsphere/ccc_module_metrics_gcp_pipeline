...
# Helper: get sib/child loop columns in numeric order
...
# Single non-loop item (Mom and Dad)
...
family <- data_tib_base %>%
  ...
  mutate(
    ...
    #### FINAL Cancer Status
      # PR2 logic determination:
        # If ANY family member has cancer, this should be yes
        # Else, if ANY question was skipped, set the value to skipped
        # Else, if ANY family members cancer status is unknown, this should be unknown
        # IF ALL family members do not have cancer, this should be no
    status = case_when(
...
#Total row
total_all_fam <- dt_fam_canc_summary %>% 
  dplyr::summarise(status = "Total",
                   n = sum(n),
                   percentage = sum(percentage))

# N (P%) Setup
dt_fam_canc_summary <- rbind(dt_fam_canc_summary, total_all_fam) %>% 
  mutate(n = scales::comma(n),
         np = paste0(n, " (", percentage, "%)")) %>% 
  select(status, np)


total_rows <- nrow(dt_fam_canc_summary)
knitr::kable(dt_fam_canc_summary , 
             caption='Any Primary Relative(s) Diagnosed With Cancer', 
             row.names=FALSE,align=c("l","c"),
             col.names=c("Cancer Status", "N (P%)"),
             booktabs = TRUE) %>%  
  add_indent(seq(1, total_rows - 1))  %>% 
  kable_styling(latex_options = c("scale_down","hold_position"))  %>% 
  footnote(general="Primary relatives consist of parents, siblings, and children.",
           general_title = "")
...
dt_fam_canc_by_relative <- family %>%
  ...
  ungroup() %>%
  mutate(cell = paste0(scales::comma(n), " (", round(percentage, 2), "%)")) %>%
...
total_row <- family %>%
  ...
  mutate(
    total = sum(n),
    status = factor("Total", levels = c("Yes", "No", "Don't Know", "Skipped this Question", "Total")),
    cell = paste0(scales::comma(n), " (100%)")
  ) %>%
  select(relative, status, cell) %>%
  pivot_wider(names_from = relative, values_from = cell)

dt_fam_canc_by_relative <- bind_rows(dt_fam_canc_by_relative, total_row)



total_rows <- nrow(dt_fam_canc_by_relative)
knitr::kable(dt_fam_canc_by_relative , 
             caption='Which Primary Relative(s) Diagnosed With Cancer', 
             row.names=FALSE,align=c("l",rep("c",4)),
             col.names=c("Cancer Status", "Mom", "Dad", "Sibling", "Child"),
             booktabs = TRUE) %>%  
  add_indent(seq(1, total_rows - 1))  %>% 
  kable_styling(latex_options = c("scale_down","hold_position")) %>% 
  footnote(general="Note: Not all participants have siblings and/or children. The denominator for these columns will differ accordingly.",
           general_title = "")

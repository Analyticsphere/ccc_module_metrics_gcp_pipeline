...
#Completed QOL before or on the same day as BL survey
BL_QOL <- trigger_time %>%
  filter(qolcomplt == "Submitted") %>%
...
\newpage
\FloatBarrier
```{r BL_After_CES, include=FALSE}


BL_CES <- all_data %>%  filter(CES_status=="Submitted") %>%
  mutate(
    BOH = as.Date(d_199471989) <= as.Date(d_517311251),
    MRE = as.Date(d_199471989) <= as.Date(d_832139544),
    SAS = as.Date(d_199471989) <= as.Date(d_770257102),
    LAW = as.Date(d_199471989) <= as.Date(d_264644252),
    BUM = as.Date(d_199471989) <= as.Date(d_222161762),
    BU = as.Date(d_199471989) <= as.Date(d_764863765),
    MW = as.Date(d_199471989) <= as.Date(d_195145666),
    COVID = as.Date(d_199471989) <= as.Date(d_784810139)
  ) %>%
    mutate(
        No_Survey_Submitted_After_CES = rowSums(select(., BOH:COVID)) == 0
    ) %>%
    pivot_longer(
        cols = c(BOH, MRE, SAS, LAW, BUM, BU, MW, COVID, No_Survey_Submitted_After_CES),
        names_to = "after_CES",
        values_to = "is_in_timeframe"
    ) %>%
  filter(is_in_timeframe) %>%  # Keep only TRUE rows
...
BL_CES_table <- BL_CES_table %>% 
  ...
  rename(`Survey Completed After CES` = after_CES, `N%` = N_percent)


knitr::kable(BL_CES_table[c(1,6,8,5,3,2,7,4,9),],
...
#Completed CES before or on the same day as BL survey
BL_CES <- trigger_time %>%
  filter(CES_status == "Submitted") %>%
  mutate(BOH = as.Date(d_199471989) <= as.Date(d_517311251),
         MRE = as.Date(d_199471989) <= as.Date(d_832139544),
         SAS = as.Date(d_199471989) <= as.Date(d_770257102),
         LAW = as.Date(d_199471989) <= as.Date(d_264644252),
         BUM = (as.Date(BUM_time) < as.Date("2024-09-30") ) & (as.Date(d_199471989) <= as.Date(d_222161762)),  
         BU = (as.Date(BU_time) < as.Date("2024-09-30")  ) & (as.Date(d_199471989) <= as.Date(d_764863765)), 
...
trig_csh <- bq_project_query(project,
                             query="SELECT distinct(Connect_ID), notification_time as CSH_time FROM `nih-nci-dceg-connect-prod-6d04.FlatConnect.notifications_JP` notif
...
CSH_notif_table = trigger_time %>%  
  ...
  mutate(time = as.numeric(difftime(as.POSIXct(ymd_hms(d_389890053)), as.POSIXct(ymd_hms(notification_time)), units="days"))) %>% 
  dplyr::summarize('N'=n(),
                   Min = round(min(time, na.rm = TRUE), digits=2),
                   Q1 = round(quantile(time, 0.25, na.rm = TRUE), digits=2),
                   Median = round(median(time, na.rm = TRUE), digits=2),
...
\newpage
\FloatBarrier
```{r BL_After_CSH, include=FALSE}


BL_CSH <- all_data %>%  filter(CSH_status=="Submitted") %>%
  mutate(after_CSH = case_when(as.Date(d_389890053) <= as.Date(d_517311251) ~ "BOH",
                               as.Date(d_389890053) <= as.Date(d_832139544) ~ "MRE",
                               as.Date(d_389890053) <= as.Date(d_770257102) ~ "SAS",
                               as.Date(d_389890053) <= as.Date(d_264644252) ~ "LAW",
                               as.Date(d_389890053) <= as.Date(d_222161762) ~ "BUM",
                               as.Date(d_389890053) <= as.Date(d_764863765) ~ "BU",
                               as.Date(d_389890053) <= as.Date(d_195145666) ~ "MW",
                               as.Date(d_389890053) <= as.Date(d_784810139) ~ "COVID",
                               TRUE ~ "No Survey Submitted After CSH"),
         after_CSH = factor(after_CSH, levels=c("BOH","MRE","SAS", 
                                                "LAW", "BUM", "BU", 
                                                "MW", "COVID", "No Survey Submitted After CSH"))) %>% 
  group_by(after_CSH) %>% tally() %>% 
  mutate(Percentage = (n / sum(n)) * 100)

summary_row <- data.frame(after_CSH = "Total Submitted CSH Survey",
                          n = sum(BL_CSH$n),
                          Percentage = 100)


BL_CSH_table <- rbind(BL_CSH, summary_row) %>%
  mutate(N_percent = paste0(formatC(n, big.mark = ",", format = "d"), 
                            " (", round(Percentage, digits = 1), "%)"))

BL_CSH_table <- BL_CSH_table %>% 
  select(after_CSH, N_percent) %>%
  rename(`Survey Completed After CSH` = after_CSH, `N%` = N_percent)


knitr::kable(BL_CSH_table,
             caption = 'Table 5.5. Baseline Survey Completion On Day of or After CSH Survey Completion', 
             booktabs=T, linesep = "", row.names=FALSE, align=c("l", "c")) %>% 
  add_indent(c(1:(nrow(BL_CSH_table)-1))) %>%  
  kable_styling(latex_options = "scale_down") #c("scale_down","hold_position"))
...
#Completed CSH before or on the same day as BL survey
BL_CSH <- trigger_time %>%
  filter(CSH_status == "Submitted") %>%
  mutate(BOH = as.Date(d_389890053) <= as.Date(d_517311251),
         MRE = as.Date(d_389890053) <= as.Date(d_832139544),
         SAS = as.Date(d_389890053) <= as.Date(d_770257102),
         LAW = as.Date(d_389890053) <= as.Date(d_264644252),
         BUM = (as.Date(BUM_time) < as.Date(CSH_time) ) & (as.Date(d_389890053) <= as.Date(d_222161762)),  
         BU = (as.Date(BU_time) < as.Date(CSH_time) ) & (as.Date(d_389890053) <= as.Date(d_764863765)), 
         MW = (as.Date(d_173836415_d_266600170_d_319972665_d_661940160) < as.Date(CSH_time)) & (as.Date(d_389890053) <= as.Date(d_195145666)),
         COVID = (as.Date(d_556788178) < as.Date(CSH_time)) & (as.Date(d_556788178) >= as.Date("2023-07-05")) & (as.Date(d_389890053) <= as.Date(d_784810139))) %>%
  pivot_longer(
    cols = c(BOH, MRE, SAS, LAW, BUM, BU, MW, COVID),
...
BL_time_eligible2 <- trigger_time %>%
  ...
  mutate( BOH = is.na(d_517311251) | (as.Date(CSH_time) <= as.Date(d_517311251)),
          MRE = !is.na(d_517311251) & (is.na(d_832139544) | as.Date(CSH_time) <= as.Date(d_832139544)),
          SAS = !is.na(d_517311251) & (is.na(d_770257102) | as.Date(CSH_time) <= as.Date(d_770257102)),
          LAW = !is.na(d_517311251) & (is.na(d_264644252) | as.Date(CSH_time) <= as.Date(d_264644252)),
          BUM = (as.Date(BUM_time) < as.Date(CSH_time))  & (is.na(d_222161762) | as.Date(CSH_time) <= as.Date(d_222161762)),
          BU = (as.Date(BU_time) < as.Date(CSH_time))  & (is.na(d_764863765) | as.Date(CSH_time) <= as.Date(d_764863765)),
          MW = (as.Date(d_173836415_d_266600170_d_319972665_d_661940160) < as.Date(CSH_time)) &
            (is.na(d_195145666) | as.Date(CSH_time) <= as.Date(d_195145666)),
          COVID = as.Date(d_556788178) < as.Date(CSH_time) &
            as.Date(d_556788178) >= as.Date("2023-07-05") &
            (is.na(d_784810139) | as.Date(CSH_time) <= as.Date(d_784810139))) %>%
  pivot_longer(
    cols = c(BOH, MRE, SAS, LAW, BUM, BU, MW, COVID),
    names_to = "CSH_timeframe",
    values_to = "is_in_timeframe"

...
cancer_loc_occ <- cancer_loc %>% 
  group_by(Connect_ID) %>%
  filter(d_793981056 == max(d_793981056, na.rm = TRUE)) %>% #highest cancer occurance count
  ungroup() %>% 
...
Diag_Verif <- cancer_loc %>%
  mutate(months_diff =difftime(as.Date(d_345545422), as.Date(d_914594314), units = "weeks")*.25,
         time_diff = case_when( months_diff < 0 ~ "Prior to verification",
                                ...
                                months_diff < 13 ~ "6-12 months post-veriification",
                                months_diff <=24 ~ "12-24 months post-veriification",
  ...
  mutate(time_diff = factor(time_diff,
                                      levels = c("Prior to verification","Within 6 months post-verification","6-12 months post-veriification",
                                                 "12-24 months post-veriification","More than 24 months post verification","Missing Verification or Diagnosis Date")))
...
tbd_table <- knitr::kable(Diag_Verif_Table, 
               caption = "Table 6. Diagnosis Date Realtive to Verification Date Among Cancer Occurences", 
               col.names = c("Number of Occurrence", "Number of Participants"), 
               row.names = FALSE, align = c("l", "c"), digits = 1, booktabs = TRUE) %>%  

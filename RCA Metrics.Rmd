...
#### Post Verif Occurrences
post_verif_counts <- rca %>% 
  filter(!is.na(Site) & d_525972260==353358909 & 
           as.Date(d_345545422) > as.Date(d_914594314)) %>% 
  group_by(Site) %>%  tally()

post_verif_counts_pts <- post_verif_counts %>% 
  mutate(counts = paste0(post_verif_counts$n, " (", round(post_verif_counts$n/(sum(post_verif_counts$n))*100, digits=1), "%)")) %>% 
  select(Site, counts)

post_verif_counts_pts[(nrow(post_verif_counts_pts)+1),] <- list("Total", paste0(sum(post_verif_counts$n), " (100%)"))


#### Combine
all_counts_pts_split <- merge(all_counts_pts, post_verif_counts_pts, by="Site", sort = FALSE)


knitr::kable(all_counts_pts_split, 
             caption='Table 1. Number of Cancer Occurrences by Healthcare System', 
             row.names=FALSE,align=c("l","c","c"),
             col.names=c("Site", "All Occurrences", "Post-Verif Occurrences"),
             booktabs = TRUE) %>%  
  add_indent(seq(1, (nrow(all_counts_pts_split) - 1)))  %>% 
...
#Number of occurrences header
header_occ <- ncol(occ_by_site)-2
...
knitr::kable(occ_by_site, 
  ...
  add_header_above(c(" " = 1, "Number of Cancer Occurrences" = header_occ,  " "=1)) %>% 
  kable_styling(latex_options = "scale_down")
...
Diag_Verif <- cancer_loc %>%
  mutate(months_diff = difftime(as.Date(d_345545422), as.Date(d_914594314), unit="weeks")/4,
         # assuming 4 weeks per month for consistency -- time_length in months throws counts off significantly
...
### Second version of the plot to remove UC Oct 2025 week
UC_excl <- sub_month %>% filter(n!='174')

#By Count
ggplot(UC_excl, aes(x = as.Date(sub_date), y=n, color=Site)) +
  geom_line() + 
  scale_y_continuous(name = "Number of Occurrences",
                     limits = c(0, NA),  expand = c(0, 0),            #start at (0,0)
                     breaks = scales::pretty_breaks(),                # nice spacing
  ...
  scale_color_manual(values=c("Baylor Scott and White Health"="#B1C7D9", "KP Colorado"="#164C71", "KP Northwest"="#FDBE19", 
                     ...
                     guide = guide_legend(nrow = 3, byrow = TRUE)) + 
  labs(color = "Site") +
  ggtitle(label = "Figure 3.1: Number of Cases Submitted Each Month by Site (Excluding UC 10/2025)") +
  theme(panel.background = element_blank(),
        legend.title = element_text(size = 8),
        legend.position = "bottom",
        axis.line = element_line(size = 0.2),
        ...
        plot.title = element_text(hjust = 0.5, size = 12, face = "bold")) +
  guides(fill = guide_legend(nrow = 2, byrow = TRUE)) + 
  labs(caption = "Note: Cancer cases submitted by UChicago in October 2025 are excluded. See Figure 3 for this information.") +
  theme(plot.caption = element_text(hjust = 0))
